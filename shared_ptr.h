...
namespace eastl
{
	...
	template <typename T>
	inline bool operator==(const shared_ptr<T>& a, std::nullptr_t) EASTL_NOEXCEPT
	{
		return !a;
	}
	...
	#if defined(EA_COMPILER_HAS_THREE_WAY_COMPARISON)
	template <typename T>
	inline std::strong_ordering operator<=>(const shared_ptr<T>& a, std::nullptr_t) EASTL_NOEXCEPT
	{
		return a.get() <=> nullptr;
	}
	#else
	template <typename T>
	inline bool operator==(std::nullptr_t, const shared_ptr<T>& b) EASTL_NOEXCEPT
	{
		return !b;
	}
	...
	template <typename T, typename U>
	inline shared_ptr<T> reinterpret_pointer_cast(shared_ptr<U> const& sharedPtr) EASTL_NOEXCEPT
	{
	...
	///                 where w is the return value.
	template <typename T, typename U> 
	inline shared_ptr<T> static_pointer_cast(const shared_ptr<U>& sharedPtr) EASTL_NOEXCEPT
	{
	...
	inline shared_ptr<T> static_shared_pointer_cast(const shared_ptr<U>& sharedPtr) EASTL_NOEXCEPT
	...
	///                 where w is the return value.
	template <typename T, typename U> 
	inline shared_ptr<T> const_pointer_cast(const shared_ptr<U>& sharedPtr) EASTL_NOEXCEPT
	{
		return shared_ptr<T>(sharedPtr, const_cast<T*>(sharedPtr.get()));
	}

	template <typename T, typename U> // Retained for support for pre-C++11 shared_ptr.
	inline shared_ptr<T> const_shared_pointer_cast(const shared_ptr<U>& sharedPtr) EASTL_NOEXCEPT
		...
		/// 
		template <typename T, typename U>
		inline shared_ptr<T> dynamic_pointer_cast(const shared_ptr<U>& sharedPtr) EASTL_NOEXCEPT
		{
		...
		dynamic_shared_pointer_cast(const shared_ptr<U>& sharedPtr) EASTL_NOEXCEPT
	...
	{ 
		size_t operator()(const shared_ptr<T>& x) const EASTL_NOEXCEPT
	...
	public:
		...
		/// weak_ptr
		/// Construction with self type.
		weak_ptr(const this_type& weakPtr) EASTL_NOEXCEPT
			: mpValue(weakPtr.mpValue),
			  mpRefCount(weakPtr.mpRefCount)
		{
			if(mpRefCount)
		...
		weak_ptr(this_type&& weakPtr) EASTL_NOEXCEPT
			: mpValue(weakPtr.mpValue),
			  mpRefCount(weakPtr.mpRefCount)
		{
		...
		template <typename U>
		weak_ptr(const weak_ptr<U>& weakPtr, typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value>::type* = 0) EASTL_NOEXCEPT
			: mpValue(weakPtr.mpValue),
				mpRefCount(weakPtr.mpRefCount)
		{
			if(mpRefCount)
		...
		/// operator=(weak_ptr)
		/// assignment to self type.
		this_type& operator=(const this_type& weakPtr) EASTL_NOEXCEPT
		{
			assign(weakPtr);
			return *this;
		}
		...
		this_type& operator=(this_type&& weakPtr) EASTL_NOEXCEPT
		{
		...
		template <typename U>
		typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value, this_type&>::type
		operator=(const weak_ptr<U>& weakPtr) EASTL_NOEXCEPT
		{
			assign(weakPtr);
			return *this;
		}
		...
		template <typename U>
		typename eastl::enable_if<eastl::is_convertible<U*, element_type*>::value, this_type&>::type
		operator=(weak_ptr<U>&& weakPtr) EASTL_NOEXCEPT
		{
			weak_ptr(eastl::move(weakPtr)).swap(*this);
			return *this;
		}
		...
		shared_ptr<T> lock() const EASTL_NOEXCEPT
		{
		...
		// Returns: use_count() == 0
		bool expired() const EASTL_NOEXCEPT
		{
			return (!mpRefCount || (mpRefCount->use_count() == 0));
		}
		...
		template <typename U>
		bool less_than(const weak_ptr<U>& weakPtr) const EASTL_NOEXCEPT
		{
			return (mpRefCount < weakPtr.mpRefCount);
		}
	...
	struct owner_less< shared_ptr<T> >
	{
		...
		bool operator()(shared_ptr<T> const& a, shared_ptr<T> const& b) const EASTL_NOEXCEPT
			{ return a.owner_before(b); }

		bool operator()(shared_ptr<T> const& a, weak_ptr<T> const& b) const EASTL_NOEXCEPT
			{ return a.owner_before(b); }

		bool operator()(weak_ptr<T> const& a, shared_ptr<T> const& b) const EASTL_NOEXCEPT
			{ return a.owner_before(b); }
	};
	...
	struct owner_less< weak_ptr<T> >
	{
		...
		bool operator()(weak_ptr<T> const& a, weak_ptr<T> const& b) const EASTL_NOEXCEPT
			{ return a.owner_before(b); }
	...
	struct owner_less<void>
	{
		...
		template<typename T, typename U>
		bool operator()(shared_ptr<T> const& a, shared_ptr<U> const& b) const EASTL_NOEXCEPT
		{
			return a.owner_before(b);
		}

		template<typename T, typename U>
		bool operator()(shared_ptr<T> const& a, weak_ptr<U> const& b) const EASTL_NOEXCEPT
		{
			return a.owner_before(b);
		}

		template<typename T, typename U>
		bool operator()(weak_ptr<T> const& a, shared_ptr<U> const& b) const EASTL_NOEXCEPT
		{
			return a.owner_before(b);
		}

		template<typename T, typename U>
		bool operator()(weak_ptr<T> const& a, weak_ptr<U> const& b) const EASTL_NOEXCEPT
		{
			return a.owner_before(b);
		}
	};
