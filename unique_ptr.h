...
namespace eastl
{
	...
	public:
		...
		unique_ptr(pointer pValue, typename eastl::conditional<eastl::is_reference<deleter_type>::value, deleter_type, typename eastl::add_lvalue_reference<const deleter_type>::type>::type deleter) EASTL_NOEXCEPT
			...
			unique_ptr(this_type&& x) EASTL_NOEXCEPT
			...
			unique_ptr(unique_ptr<U, E>&& u, typename enable_if<!is_array<U>::value && is_convertible<typename unique_ptr<U, E>::pointer, pointer>::value && is_convertible<E, deleter_type>::value && (is_same<deleter_type, E>::value || !is_reference<deleter_type>::value)>::type* = 0) EASTL_NOEXCEPT
			...
			this_type& operator=(this_type&& x) EASTL_NOEXCEPT
			{
				reset(x.release());
			...
			operator=(unique_ptr<U, E>&& u) EASTL_NOEXCEPT
			{
				reset(u.release());
		...
		#else
			template <typename U, typename E>
			this_type& operator=(unique_ptr<U, E> u) EASTL_NOEXCEPT  // Pass by value.
			{
				reset(u.release());
		...
		void reset(pointer pValue = pointer()) EASTL_NOEXCEPT
		{
		...
		///    delete pInt;
		pointer release() EASTL_NOEXCEPT
		{
		...
		pointer detach() EASTL_NOEXCEPT { return release(); } 
		...
		///    X* pX = ptr.get();
		///    pX->DoSomething();
		pointer get() const EASTL_NOEXCEPT
		{
			return mPair.first();
		}
		...
		deleter_type& get_deleter() EASTL_NOEXCEPT
		{
			return mPair.second();
		}
		...
		const deleter_type& get_deleter() const EASTL_NOEXCEPT
		{
			return mPair.second();
		}
	...
	public:
		...
		explicit unique_ptr(P pArray, typename eastl::enable_if<Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0) EASTL_NOEXCEPT
			: mPair(pArray)
		{
			static_assert(!eastl::is_pointer<deleter_type>::value, "unique_ptr deleter default-constructed with null pointer. Use a different constructor or change your deleter to a class.");
		}
		...
		unique_ptr(P pArray, typename eastl::conditional<eastl::is_reference<deleter_type>::value, deleter_type,
														...
														typename eastl::enable_if<Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0) EASTL_NOEXCEPT
			: mPair(pArray, deleter) {}

		#if EASTL_MOVE_SEMANTICS_ENABLED
			template <typename P>
			unique_ptr(P pArray, typename eastl::remove_reference<deleter_type>::type&& deleter, typename eastl::enable_if<Internal::is_array_cv_convertible<P, pointer>::value>::type* = 0) EASTL_NOEXCEPT
			...
			unique_ptr(unique_ptr<U, E>&& u, typename enable_if<Internal::is_safe_array_conversion<T, pointer, U, typename unique_ptr<U, E>::pointer>::value && 
															   ...
															   (!eastl::is_reference<deleter_type>::value || eastl::is_same<E, deleter_type>::value)>::type* = 0) EASTL_NOEXCEPT
				: mPair(u.release(), eastl::forward<E>(u.get_deleter())) {}
		...
		void reset(pointer pArray = pointer()) EASTL_NOEXCEPT
		{
	...
	{
		size_t operator()(const unique_ptr<T, D>& x) const EASTL_NOEXCEPT
	...
	template <typename T, typename D>
	inline void swap(unique_ptr<T, D>& a, unique_ptr<T, D>& b) EASTL_NOEXCEPT
	{
		a.swap(b);
	}
	...
	template <typename T, typename D>
	inline bool operator==(const unique_ptr<T, D>& a, std::nullptr_t) EASTL_NOEXCEPT
	{
		return !a;
	}

	template <typename T, typename D>
	inline bool operator==(std::nullptr_t, const unique_ptr<T, D>& a) EASTL_NOEXCEPT
	{
		return !a;
	}
