...
namespace eastl
{
	...
	/// the useSelf template implementation, which is used for sets.
	///
	...
	public:
		...
		typedef typename type_select<bConstIterators, eastl::useSelf<Value>, 
	...
	public:
		...
		size_type bucket_count() const  EASTL_NOEXCEPT // This function is unnecessary, as the user can directly reference
	...
	public:
		...
		// The use for equalRange in a hash_table seems somewhat questionable.
	...
	intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::equalRange(const key_type& k)
	{
		const size_type n     = (size_type)(mHash(k) % kBucketCount);
	...
	intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::equalRange(const key_type& k) const
	{
		const size_type n     = (size_type)(mHash(k) % kBucketCount);
	...
	template <typename K, typename V, typename H, typename Eq, size_t bC, bool bM, bool bU>
	int intrusive_hashtable<K, V, H, Eq, bC, bM, bU>::validateIterator(const_iterator i) const
	{
