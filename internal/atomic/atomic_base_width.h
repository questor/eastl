...
#define EASTL_ATOMIC_STORE_FUNC_IMPL(op, bits)							\
	EASTL_PREPROCESSOR_JOIN(op, bits)(T, this->GetAtomicAddress(), desired);

#define EASTL_ATOMIC_LOAD_FUNC_IMPL(op, bits)					\
	T retVal;													\
	EASTL_PREPROCESSOR_JOIN(op, bits)(T, retVal, this->GetAtomicAddress()); \
	return retVal;

#define EASTL_ATOMIC_EXCHANGE_FUNC_IMPL(op, bits)						\
	T retVal;															\
	EASTL_PREPROCESSOR_JOIN(op, bits)(T, retVal, this->GetAtomicAddress(), desired); \
	return retVal;

#define EASTL_ATOMIC_CMPXCHG_FUNC_IMPL(op, bits)						\
	bool retVal;														\
	EASTL_PREPROCESSOR_JOIN(op, bits)(T, retVal, this->GetAtomicAddress(), &expected, desired); \
	return retVal;
...
#define EASTL_ATOMIC_BASE_WIDTH_SPECIALIZE(bytes, bits)					\
	...
	private:															\
																		\
		static_assert(EASTL_ALIGN_OF(atomic_size_aligned<T>) == bytes, "eastl::atomic<T> must be sizeof(T) aligned!"); \
		static_assert(EASTL_ALIGN_OF(atomic_size_aligned<T>) == sizeof(T), "eastl::atomic<T> must be sizeof(T) aligned!"); \
		using Base = atomic_size_aligned<T>;							\
																		\
	public: /* ctors */													\
																		\
		atomic_base_width(T desired) EASTL_NOEXCEPT						\
			: Base{ desired }											\
		{																\
		}																\
																		\
		atomic_base_width() EASTL_NOEXCEPT = default;						\
																		\
		atomic_base_width(const atomic_base_width&) EASTL_NOEXCEPT = delete; \
																		\
	public: /* store */													\
																		\
		using Base::store;												\
																		\
		void store(T desired) EASTL_NOEXCEPT								\
		{																\
			EASTL_ATOMIC_STORE_FUNC_IMPL(EASTL_ATOMIC_STORE_SEQ_CST_, bits); \
		}																\
																		\
		void store(T desired, eastl::internal::memory_order_relaxed_s) EASTL_NOEXCEPT	\
		{																\
			EASTL_ATOMIC_STORE_FUNC_IMPL(EASTL_ATOMIC_STORE_RELAXED_, bits); \
		}																\
																		\
		void store(T desired, eastl::internal::memory_order_release_s) EASTL_NOEXCEPT	\
		{																\
			EASTL_ATOMIC_STORE_FUNC_IMPL(EASTL_ATOMIC_STORE_RELEASE_, bits); \
		}																\
																		\
		void store(T desired, eastl::internal::memory_order_seq_cst_s) EASTL_NOEXCEPT	\
		{																\
			EASTL_ATOMIC_STORE_FUNC_IMPL(EASTL_ATOMIC_STORE_SEQ_CST_, bits); \
		}																\
																		\
	public: /* load */													\
		...
		using Base::load;												\
																		\
		T load() const EASTL_NOEXCEPT										\
		{																\
			EASTL_ATOMIC_LOAD_FUNC_IMPL(EASTL_ATOMIC_LOAD_SEQ_CST_, bits); \
		}																\
																		\
		T load(eastl::internal::memory_order_relaxed_s) const EASTL_NOEXCEPT \
		{																\
			EASTL_ATOMIC_LOAD_FUNC_IMPL(EASTL_ATOMIC_LOAD_RELAXED_, bits); \
		}																\
																		\
		T load(eastl::internal::memory_order_acquire_s) const EASTL_NOEXCEPT \
		{																\
			EASTL_ATOMIC_LOAD_FUNC_IMPL(EASTL_ATOMIC_LOAD_ACQUIRE_, bits); \
		}																\
																		\
		T load(eastl::internal::memory_order_seq_cst_s) const EASTL_NOEXCEPT \
		{																\
			EASTL_ATOMIC_LOAD_FUNC_IMPL(EASTL_ATOMIC_LOAD_SEQ_CST_, bits); \
		}																\
																		\
	public: /* exchange */												\
		...
		using Base::exchange;											\
																		\
		T exchange(T desired) EASTL_NOEXCEPT								\
		{																\
			EASTL_ATOMIC_EXCHANGE_FUNC_IMPL(EASTL_ATOMIC_EXCHANGE_SEQ_CST_, bits); \
		}																\
																		\
		T exchange(T desired, eastl::internal::memory_order_relaxed_s) EASTL_NOEXCEPT \
		{																\
			EASTL_ATOMIC_EXCHANGE_FUNC_IMPL(EASTL_ATOMIC_EXCHANGE_RELAXED_, bits); \
		}																\
																		\
		T exchange(T desired, eastl::internal::memory_order_acquire_s) EASTL_NOEXCEPT	\
		{																\
			EASTL_ATOMIC_EXCHANGE_FUNC_IMPL(EASTL_ATOMIC_EXCHANGE_ACQUIRE_, bits); \
		}																\
																		\
		T exchange(T desired, eastl::internal::memory_order_release_s) EASTL_NOEXCEPT	\
		{																\
			EASTL_ATOMIC_EXCHANGE_FUNC_IMPL(EASTL_ATOMIC_EXCHANGE_RELEASE_, bits); \
		}																\
																		\
		T exchange(T desired, eastl::internal::memory_order_acq_rel_s) EASTL_NOEXCEPT	\
		{																\
			EASTL_ATOMIC_EXCHANGE_FUNC_IMPL(EASTL_ATOMIC_EXCHANGE_ACQ_REL_, bits); \
		}																\
																		\
		T exchange(T desired, eastl::internal::memory_order_seq_cst_s) EASTL_NOEXCEPT	\
		{																\
			EASTL_ATOMIC_EXCHANGE_FUNC_IMPL(EASTL_ATOMIC_EXCHANGE_SEQ_CST_, bits); \
		}																\
																		\
	...
	public: /* assignment operator */									\
		...
		using Base::operator=;											\
																		\
		T operator =(T desired) EASTL_NOEXCEPT								\
		{																\
			store(desired, eastl::memory_order_seq_cst);				\
			return desired;												\
		}																\
																		\
		atomic_base_width& operator =(const atomic_base_width&)          EASTL_NOEXCEPT = delete; \
		atomic_base_width& operator =(const atomic_base_width&) volatile EASTL_NOEXCEPT = delete; \
																		\
	};
