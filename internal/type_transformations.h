...
namespace eastl
{
	...
	// variables. The EABase EASTL_ALIGN_MAX_AUTOMATIC define identifies the 
	...
	#define EASTL_TYPE_TRAIT_aligned_storage_CONFORMANCE 1    // aligned_storage is conforming.
		...
		{
			struct type { unsigned char mCharData[N]; } EASTL_ALIGN(Align);
		};
		...
		// Some compilers don't allow you to to use EASTL_ALIGNED with anything by a numeric literal, 
		...
		template<size_t N> struct aligned_storage_helper<N,    2> { struct EASTL_ALIGN(   2) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N,    4> { struct EASTL_ALIGN(   4) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N,    8> { struct EASTL_ALIGN(   8) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N,   16> { struct EASTL_ALIGN(  16) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N,   32> { struct EASTL_ALIGN(  32) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N,   64> { struct EASTL_ALIGN(  64) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N,  128> { struct EASTL_ALIGN( 128) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N,  256> { struct EASTL_ALIGN( 256) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N,  512> { struct EASTL_ALIGN( 512) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N, 1024> { struct EASTL_ALIGN(1024) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N, 2048> { struct EASTL_ALIGN(2048) type{ unsigned char mCharData[N]; }; };
		template<size_t N> struct aligned_storage_helper<N, 4096> { struct EASTL_ALIGN(4096) type{ unsigned char mCharData[N]; }; };
		...
		struct aligned_storage
		{
			union type
			{
				...
				struct EASTL_ALIGN(Align) mStruct{ }; 
			};
		};
	#endif
	...
	///////////////////////////////////////////////////////////////////////
		...
		struct aligned_union
		{
			...
			static const size_t alignment0      = eastl::static_max<EASTL_ALIGN_OF(Type0), EASTL_ALIGN_OF(Type1)>::value;
			static const size_t alignment1      = eastl::static_max<alignment0,         EASTL_ALIGN_OF(Type2)>::value;
			static const size_t alignment_value = eastl::static_max<alignment1,         EASTL_ALIGN_OF(Type3)>::value;
		...
		struct aligned_union
		{
			...
			static const size_t alignment_value = eastl::static_max<EASTL_ALIGN_OF(Type0), EASTL_ALIGN_OF(TypeN)...>::value;
	...
	DestType union_cast(SourceType sourceValue)
	{
		EASTL_CT_ASSERT((sizeof(DestType) == sizeof(SourceType)) && 
						(EASTL_ALIGN_OF(DestType) == EASTL_ALIGN_OF(SourceType)));               // To support differening alignments, we would need to use a memcpy-based solution or find a way to make the two union members align with each other.
