...
namespace eastl
{
	...
	struct aligned_buffer<size, 2>    { EASTL_PREFIX_ALIGN(2) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(2); };

	template<size_t size>
	struct aligned_buffer<size, 4>    { EASTL_PREFIX_ALIGN(4) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(4); };

	template<size_t size>
	struct aligned_buffer<size, 8>    { EASTL_PREFIX_ALIGN(8) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(8); };

	template<size_t size>
	struct aligned_buffer<size, 16>   { EASTL_PREFIX_ALIGN(16) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(16); };

	template<size_t size>
	struct aligned_buffer<size, 32>   { EASTL_PREFIX_ALIGN(32) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(32); };

	template<size_t size>
	struct aligned_buffer<size, 64>   { EASTL_PREFIX_ALIGN(64) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(64); };

	template<size_t size>
	struct aligned_buffer<size, 128>  { EASTL_PREFIX_ALIGN(128) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(128); };

	template<size_t size>
	struct aligned_buffer<size, 256>  { EASTL_PREFIX_ALIGN(256) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(256); };

	template<size_t size>
	struct aligned_buffer<size, 512>  { EASTL_PREFIX_ALIGN(512) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(512); };

	template<size_t size>
	struct aligned_buffer<size, 1024> { EASTL_PREFIX_ALIGN(1024) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(1024); };

	template<size_t size>
	struct aligned_buffer<size, 2048> { EASTL_PREFIX_ALIGN(2048) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(2048); };

	template<size_t size>
	struct aligned_buffer<size, 4096> { EASTL_PREFIX_ALIGN(4096) aligned_buffer_char buffer[size] EASTL_POSTFIX_ALIGN(4096); };
	...
	public:
		...
		const overflow_allocator_type& getOverflowAllocator() const
		{
		...
		void setOverflowAllocator(const overflow_allocator_type& overflowAllocator)
		{
	...
	// and the getOverflowAllocator / setOverflowAllocator functions.
	...
	class fixedHashtableAllocator
	{
	public:
		typedef typename type_select<bEnableOverflow, fixed_pool_with_overflow<OverflowAllocator>, fixed_pool>::type                                 pool_type;
		typedef fixedHashtableAllocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>  this_type;
		typedef OverflowAllocator overflow_allocator_type;
	...
	public:
		...
		//fixedHashtableAllocator(const char* pName)
		//{
		//    mPool.setName(pName);
		//}

		fixedHashtableAllocator(void* pNodeBuffer)
		...
		fixedHashtableAllocator(void* pNodeBuffer, const overflow_allocator_type& allocator)
		...
		fixedHashtableAllocator(void* pNodeBuffer, void* pBucketBuffer)
			: mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset),
		...
		fixedHashtableAllocator(void* pNodeBuffer, void* pBucketBuffer, const overflow_allocator_type& allocator)
			: mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset, allocator),
			  mpBucketBuffer(pBucketBuffer)
		{
		}
		...
		///
		fixedHashtableAllocator(const this_type& x)
		...
		fixedHashtableAllocator& operator=(const fixedHashtableAllocator& x)
		{
			mPool = x.mPool;
			return *this;
		}
	...
	class fixedHashtableAllocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator>
	{
	public:
		typedef fixed_pool pool_type;
		typedef fixedHashtableAllocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator>  this_type;
		typedef OverflowAllocator overflow_allocator_type;
	...
	public:
		...
		fixedHashtableAllocator(void* pNodeBuffer, const overflow_allocator_type& /*allocator*/) // allocator is unused because bEnableOverflow is false in this specialization.
			: mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset),
			  mpBucketBuffer(NULL)
		{
			// EASTL_ASSERT(false); // As it stands now, this is not supposed to be called.
		...
		fixedHashtableAllocator(void* pNodeBuffer, void* pBucketBuffer, const overflow_allocator_type& /*allocator*/) // allocator is unused because bEnableOverflow is false in this specialization.
			: mPool(pNodeBuffer, kBufferSize, kNodeSize, kNodeAlignment, kNodeAlignmentOffset),
			  mpBucketBuffer(pBucketBuffer)
		{
		}
		...
		/// See the discussion above in fixed_node_allocator for important information about this.
		///
		fixedHashtableAllocator(const this_type& x)   // No need to copy the overflow allocator, because bEnableOverflow is false in this specialization.
	...
	inline bool operator==(const fixedHashtableAllocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>& a, 
						   const fixedHashtableAllocator<bucketCount, nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>& b)
	{
		return (&a == &b); // They are only equal if they are the same object.
	}
	...
	///
	template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment, size_t nodeAlignmentOffset, bool bEnableOverflow, typename OverflowAllocator = EASTLAllocatorType>
	class fixedVector_allocator
	{
	public:
		typedef fixedVector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>  this_type;
		typedef OverflowAllocator overflow_allocator_type;
	...
	public:
		// Disabled because it causes compile conflicts.
		//fixedVector_allocator(const char* pName = NULL)
		//{
		//    mOverflowAllocator.setName(pName);
		//}
		...
		fixedVector_allocator(void* pNodeBuffer, const overflow_allocator_type& allocator)
			: mOverflowAllocator(allocator), mpPoolBegin(pNodeBuffer)
		{
		}

		// Disabled because the default is sufficient.
		//fixedVector_allocator(const fixedVector_allocator& x)
		//{
		//    mpPoolBegin        = x.mpPoolBegin;
		//    mOverflowAllocator = x.mOverflowAllocator;
		//}

		fixedVector_allocator& operator=(const fixedVector_allocator& x)
		{
	...
	template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment, size_t nodeAlignmentOffset, typename OverflowAllocator>
	class fixedVector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator>
	{
	public:
		typedef fixedVector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, false, OverflowAllocator>  this_type;
		typedef OverflowAllocator overflow_allocator_type;
		...
		fixedVector_allocator(void* /*pNodeBuffer*/, const overflow_allocator_type& /*allocator*/)  // allocator is unused because bEnableOverflow is false in this specialization.
		{
		}
		...
		// fixedVector_allocator(const fixedVector_allocator&)
		// {
		// }
		...
		void* allocate(size_t /*n*/, int /*flags*/ = 0)
		{
			EASTL_ASSERT(false); // A fixedVector should not reallocate, else the user has exhausted its space.
			return NULL;
		}
	...
	template <size_t nodeSize, size_t nodeCount, size_t nodeAlignment, size_t nodeAlignmentOffset, bool bEnableOverflow, typename OverflowAllocator>
	inline bool operator==(const fixedVector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>& a, 
						   const fixedVector_allocator<nodeSize, nodeCount, nodeAlignment, nodeAlignmentOffset, bEnableOverflow, OverflowAllocator>& b)
	{
		return (&a == &b); // They are only equal if they are the same object.
	}
	...
	class fixedSwap_impl<Container, false>
	{
	public:
		static void swap(Container& a, Container& b)
		{
	...
	template <typename Container>
	class fixedSwap_impl<Container, true>
	{
	public:
		static void swap(Container& a, Container& b)
		{
	...
	template<typename Container>
	void fixedSwap(Container& a, Container& b)
	{
		return fixedSwap_impl<Container, sizeof(Container) >= EASTL_MAX_STACK_USAGE>::swap(a, b);
	}
