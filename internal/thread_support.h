...
namespace eastl
{
	namespace Internal
	{
		...
		inline int32_t atomic_increment(int32_t* p32) EASTL_NOEXCEPT
		{
			#if defined(EA_COMPILER_CLANG) || (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4003))
		...
		/// atomic_decrement
		/// Returns the new value.
		inline int32_t atomic_decrement(int32_t* p32) EASTL_NOEXCEPT
		{
			#if defined(EA_COMPILER_CLANG) || (defined(EA_COMPILER_GNUC) && (EA_COMPILER_VERSION >= 4003))
				return __sync_add_and_fetch(p32, -1);
			#elif defined(EA_COMPILER_MSVC)
		...
		{
		public:
			EASTL_FORCE_INLINE auto_mutex(mutex& mutex) : pMutex(&mutex)
				{ pMutex->lock(); }

			EASTL_FORCE_INLINE ~auto_mutex()
				{ pMutex->unlock(); }
