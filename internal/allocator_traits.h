...
namespace eastl
{
	...
	namespace Internal
	{
		...
		// has_maxSize
        template <class Alloc>
        auto has_maxSize_test(Alloc&& a) -> decltype(a.maxSize(), eastl::true_type());

        template <class Alloc>
        auto has_maxSize_test(const volatile Alloc& a) -> eastl::false_type;

        template <class Alloc>
        struct has_maxSize
            : public eastl::integral_constant<bool,
                                is_same<decltype(has_maxSize_test(eastl::declval<Alloc&>())), eastl::true_type>::value>
        {
        };
	...
	struct allocator_traits
	{
	    ...
	    static size_type internal_maxSize(true_type, const allocator_type& a) { return a.maxSize(); }
	    static size_type internal_maxSize(false_type, const allocator_type&) { return (eastl::numeric_limits<size_type>::max)(); }  // additional parenthesis disables the windows max macro from expanding. 
	    static size_type maxSize(const allocator_type& a) EASTL_NOEXCEPT 
	    {
		    return internal_maxSize(Internal::has_maxSize<const allocator_type>(), a);
	    }
		...
		static void deallocate(allocator_type& a, pointer p, size_type n) EASTL_NOEXCEPT { a.deallocate(p, n); }
