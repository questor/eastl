...
namespace eastl
{
	...
	struct default_delete
	{
			...
			EA_CONSTEXPR default_delete() EASTL_NOEXCEPT {}
		...
		#else
			EA_CONSTEXPR default_delete() EASTL_NOEXCEPT = default;
		#endif

		template <typename U>  // Enable if T* can be constructed with U* (i.e. U* is convertible to T*).
		default_delete(const default_delete<U>&, typename eastl::enable_if<is_convertible<U*, T*>::value>::type* = 0) EASTL_NOEXCEPT {}

		void operator()(T* p) const EASTL_NOEXCEPT
			{ delete p; }
	};
	...
	struct default_delete<T[]> // Specialization for arrays.
	{
		...
		default_delete(const default_delete<U[]>&, typename eastl::enable_if<Internal::is_array_cv_convertible<U*, T*>::value>::type* = 0) EASTL_NOEXCEPT {}
