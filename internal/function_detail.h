...
namespace eastl
{
		...
		{
		public:
			bad_function_call() EASTL_NOEXCEPT = default;
	...
	namespace internal
	{
		...
		public:
			...
			public:
				...
				static Functor* GetFunctorPtr(const FunctorStorageType& storage) EASTL_NOEXCEPT
				{
				...
				static void MoveFunctor(FunctorStorageType& to, FunctorStorageType& from) EASTL_NOEXCEPT
				{
					::new (GetFunctorPtr(to)) Functor(eastl::move(*GetFunctorPtr(from)));
				}

				static void* Manager(void* to, void* from, typename function_base_detail::ManagerOperations ops) EASTL_NOEXCEPT
				{
					switch (ops)
					{
			...
			public:
				...
				static Functor*& GetFunctorPtrRef(const FunctorStorageType& storage) EASTL_NOEXCEPT
				{
					return storage.template GetStorageTypeRef<Functor*>();
				}
			...
			#if EASTL_RTTI_ENABLED
				static void* GetTypeInfo() EASTL_NOEXCEPT
				{
			...
			function_base_detail() EASTL_NOEXCEPT = default;
			~function_base_detail() EASTL_NOEXCEPT = default;
		};
		...
		public:
			function_detail() EASTL_NOEXCEPT = default;
			function_detail(std::nullptr_t) EASTL_NOEXCEPT {}
			...
			~function_detail() EASTL_NOEXCEPT
			{
				Destroy();
			}
			...
			function_detail& operator=(std::nullptr_t) EASTL_NOEXCEPT
			{
				Destroy();
				mMgrFuncPtr = nullptr;
			...
			template <typename Functor>
			function_detail& operator=(eastl::reference_wrapper<Functor> f) EASTL_NOEXCEPT
			{
				Destroy();
			...
			void swap(function_detail& other) EASTL_NOEXCEPT
			{
				if(this == &other)
					return;
		...
		private:
			bool HaveManager() const EASTL_NOEXCEPT
			{
				return (mMgrFuncPtr != nullptr);
			}

			void Destroy() EASTL_NOEXCEPT
			{
				if (HaveManager())
				{
