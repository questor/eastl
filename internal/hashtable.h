...
namespace eastl
{
	...
	/// iterators. This is true for hashMap and hashMultimap,
	/// false for hashSet and hashMultiset.
	///
	/// bUniqueKeys: true if the return value of hashtable::count(k)
	/// is always at most one, false if it may be an arbitrary number. 
	/// This is true for hashSet and hashMap and is false for 
	...
	public:
		...
		// initializer_list ctor support is implemented in subclasses (e.g. hashSet).
	...
	public:
		...
		local_iterator begin(size_type n) EASTL_NOEXCEPT
			{ return local_iterator(mpBucketArray[n]); }

		const_local_iterator begin(size_type n) const EASTL_NOEXCEPT
			{ return const_local_iterator(mpBucketArray[n]); }

		const_local_iterator cbegin(size_type n) const EASTL_NOEXCEPT
			{ return const_local_iterator(mpBucketArray[n]); }

		// Returns an iterator to the last item in a bucket returned by begin(n).
		local_iterator end(size_type) EASTL_NOEXCEPT
			{ return local_iterator(NULL); }

		const_local_iterator end(size_type) const EASTL_NOEXCEPT
			{ return const_local_iterator(NULL); }

		const_local_iterator cend(size_type) const EASTL_NOEXCEPT
			{ return const_local_iterator(NULL); }
		...
		size_type bucket_count() const EASTL_NOEXCEPT
			{ return mnBucketCount; }

		size_type bucket_size(size_type n) const EASTL_NOEXCEPT
			{ return (size_type)eastl::distance(begin(n), end(n)); }

		//size_type bucket(const key_type& k) const EASTL_NOEXCEPT
	...
	public:
		...
		float load_factor() const EASTL_NOEXCEPT
		...
		const rehash_policy_type& rehash_policy() const EASTL_NOEXCEPT
	...
	public:
		...
		size_type count(const key_type& k) const EASTL_NOEXCEPT;
	...
	protected:
		iterator DoGetResultIterator(true_type, const insert_return_type& irt)  const EASTL_NOEXCEPT { return irt.first; }
		iterator DoGetResultIterator(false_type, const insert_return_type& irt) const EASTL_NOEXCEPT { return irt; }
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::getAllocator() const EASTL_NOEXCEPT
	{
		return mAllocator;
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::getAllocator() EASTL_NOEXCEPT
	{
		return mAllocator;
	}
	...
	template <typename K, typename V, typename A, typename EK, typename Eq,
			  typename H1, typename H2, typename H, typename RP, bool bC, bool bM, bool bU>
	inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::setAllocator(const allocator_type& allocator)
	{
		mAllocator = allocator;
	}
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::count(const key_type& k) const EASTL_NOEXCEPT
	{
		const hash_code_t c      = get_hash_code(k);
		const size_type   n      = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
		size_type         result = 0;
	...
	eastl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator,
				typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::iterator>
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::equalRange(const key_type& k)
	{
		const hash_code_t c     = get_hash_code(k);
		const size_type   n     = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
	...
	eastl::pair<typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::const_iterator,
				typename hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::const_iterator>
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::equalRange(const key_type& k) const
	{
		const hash_code_t c     = get_hash_code(k);
		const size_type   n     = (size_type)bucket_index(k, c, (uint32_t)mnBucketCount);
		node_type**       head  = mpBucketArray + n;
	...
	hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoFindNode(node_type* pNode, hash_code_t c) const
	{
		...
		hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::DoInsertValue(false_type, Args&&... args) // false_type means bUniqueKeys is false.
		{
			...
			// The benefit is that equalRange can work in a sensible manner and that
			...
			// us that equalRange span all equal items? 
	...
	inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::clear(bool clearBuckets)
	{
		...
		template <typename K, typename V, typename A, typename EK, typename Eq,
				  typename H1, typename H2, typename H, typename RP, bool bC, bool bM, bool bU>
		inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::reset() EASTL_NOEXCEPT
		{
			reset_lose_memory();
		}
	#endif
	...
	template <typename K, typename V, typename A, typename EK, typename Eq,
			  typename H1, typename H2, typename H, typename RP, bool bC, bool bM, bool bU>
	inline void hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::reset_lose_memory() EASTL_NOEXCEPT
	{
	...
	template <typename K, typename V, typename A, typename EK, typename Eq,
			  typename H1, typename H2, typename H, typename RP, bool bC, bool bM, bool bU>
	int hashtable<K, V, A, EK, Eq, H1, H2, H, RP, bC, bM, bU>::validateIterator(const_iterator i) const
	{
	...
	// operator==, != have been moved to the specific container subclasses (e.g. hashMap).
