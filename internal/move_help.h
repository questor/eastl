...
///
	...
	#define EASTL_MOVE_BACKWARD_RANGE(first, last, result)    eastl::copyBackward(first, last, result)
#endif
...
namespace eastl
{
		...
		//
		template <typename T>
		EA_CPP14_CONSTEXPR T&& forward(typename eastl::remove_reference<T>::type& x) EASTL_NOEXCEPT
		{
		...
		//
			...
			move_if_noexcept(T& x) EASTL_NOEXCEPT
			{
				return eastl::move(x);
			}
		#else
	...
	#else
		template <typename T>
		T& forward(T& x) EASTL_NOEXCEPT
		{
			return x;
		}

		template<typename T>
		T& move(T& x) EASTL_NOEXCEPT
		{
			return x;
		}

		template <typename T> 
		T& move_if_noexcept(T& x) EASTL_NOEXCEPT
		{
			return x;
		}
