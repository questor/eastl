...
#define FUNC_TEMPLATE_NOEXCEPT(FUNCTOR, ALLOCATOR) EASTL_NOEXCEPT_IF(EASTL_NOEXCEPT_EXPR((detail::is_inplace_allocated<FUNCTOR, ALLOCATOR>::value)))
...
namespace detail
{
	...
	struct function_table_inplace_specialization
	{
#if EASTL_VARIADIC_TEMPLATES_ENABLED 
		template<typename Result, typename... Arguments>
		EASTL_FORCE_INLINE static Result call(const functor_storage_type & storage, Arguments... arguments)
		{
		...
		// no argument case
		template<typename Result>
		EASTL_FORCE_INLINE static Result call(const functor_storage_type & storage)
		{
		...
		// single argument case
		template<typename Result, typename Argument0>
		EASTL_FORCE_INLINE static Result call(const functor_storage_type & storage, Argument0 argument0)
		{
			// do not call get_functor_ref because I want this function to be fast
			// in debug when nothing gets inlined
		...
		static void move_functor(total_storage_type & lhs, total_storage_type && rhs) EASTL_NOEXCEPT
		{
			new (&get_functor_ref(lhs)) T(EASTL_MOVE(get_functor_ref(rhs)));
		}
		static void destroy_functor(Allocator &, total_storage_type & storage) EASTL_NOEXCEPT
		{
			get_functor_ref(storage).~T();
		}
		static T & get_functor_ref(const total_storage_type & storage) EASTL_NOEXCEPT
		{
		...
		static void move_allocator(total_storage_type& lhs, total_storage_type&& rhs) EASTL_NOEXCEPT
		{
			new (&lhs.allocator_storage) Allocator(EASTL_MOVE((getAllocator(rhs))));
		}

		static Allocator& getAllocator(const total_storage_type& storage) EASTL_NOEXCEPT
		{
			return const_cast<Allocator&>(reinterpret_cast<const Allocator&>(storage.allocator_storage));
		}

		static void destroy_allocator(total_storage_type& storage) EASTL_NOEXCEPT
		{
			getAllocator(storage).~Allocator();
		}
	};
	...
	struct function_table_inplace_specialization<T, Allocator, typename eastl::enable_if<!is_inplace_allocated<T, Allocator>::value>::type>
	{
		...
		static void store_functor(total_storage_type & self, T to_store)
		{
			...
			Allocator& allocator = getAllocator(self);
		...
		static void destroy_functor(Allocator & allocator, total_storage_type & storage) EASTL_NOEXCEPT
		{
		...
		static Functor& get_functor_ref(const total_storage_type & storage) EASTL_NOEXCEPT
		{
			return *get_functor_ptr_ref(storage);
		}

		static FunctorPointer& get_functor_ptr_ref(total_storage_type & storage) EASTL_NOEXCEPT
		{
			return const_cast<FunctorPointer&>(reinterpret_cast<const FunctorPointer&>(storage.functor));
		}

		static const FunctorPointer& get_functor_ptr_ref(const total_storage_type & storage) EASTL_NOEXCEPT
		{
			return const_cast<FunctorPointer&>(reinterpret_cast<const FunctorPointer&>(storage.functor));
		}
		...
		static void destroy_allocator(total_storage_type& storage) EASTL_NOEXCEPT
		{
			auto& allocator = getAllocator(storage);
	...
	struct function_table
	{
		...
		static void templated_call_move_and_destroy(total_storage_type & lhs, total_storage_type && rhs)
		{
			...
			specialization::destroy_functor(specialization::getAllocator(rhs), rhs);
		...
		static void templated_call_copy(total_storage_type & lhs, const total_storage_type & rhs)
		{
			typedef function_table_inplace_specialization<T, Allocator> specialization;
			init_function_table<T, Allocator>(lhs);
			specialization::store_allocator(lhs, Allocator(specialization::getAllocator(rhs)));
		...
		{
			typedef function_table_inplace_specialization<T, Allocator> specialization;
			specialization::destroy_functor(specialization::getAllocator(self), self);
...
{
public:
	function() EASTL_NOEXCEPT
	{
		initialize_empty();
	}
	function(std::nullptr_t) EASTL_NOEXCEPT
	{
		initialize_empty();
	}
	function(function && other) EASTL_NOEXCEPT
	{
		initialize_empty();
		swap(other);
	}
	...
	template<typename Allocator>
	function(eastl::allocator_arg_t, const Allocator&, function&& other) EASTL_NOEXCEPT
	{
		// ignore the allocator because I don't allocate
		initialize_empty();
		swap(other);
	...
	function & operator=(function other) EASTL_NOEXCEPT
	{
		swap(other);
		return *this;
	}
...
private:
	...
	void initialize_empty() EASTL_NOEXCEPT
	{
...
template<typename T>
bool operator==(std::nullptr_t, const function<T> & rhs) EASTL_NOEXCEPT
{
	return !rhs;
}
template<typename T>
bool operator==(const function<T> & lhs, std::nullptr_t) EASTL_NOEXCEPT
{
	return !lhs;
}
template<typename T>
