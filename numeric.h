...
namespace eastl
{
	...
	///
	template <typename T>
	constexpr eastl::enable_if_t<eastl::is_arithmetic_v<T> && !eastl::is_same_v<eastl::remove_cv_t<T>, bool>, T> midpoint(const T lhs, const T rhs) EASTL_NOEXCEPT
	{
	...
	template <class T>
	constexpr T shared_lerp(const T a, const T b, const T t) EASTL_NOEXCEPT
	{
		if ((a <= 0 && b >= 0) || (a >= 0 && b <= 0))
		{
			return t * b + (1 - t) * a;
	...
	///
	constexpr float lerp(float a, float b, float t) EASTL_NOEXCEPT { return shared_lerp(a, b, t); }
	constexpr double lerp(double a, double b, double t) EASTL_NOEXCEPT { return shared_lerp(a, b, t); }
	constexpr long double lerp(long double a, long double b, long double t) EASTL_NOEXCEPT { return shared_lerp(a, b, t); }
	#endif
