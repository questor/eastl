...
namespace eastl
{
	...
	protected:
		...
		void copyBackward(const iterator& first, const iterator& last, true_type);  // true means that value_type has the type_trait has_trivial_relocate,
	...
	template <typename T, typename Pointer, typename Reference, unsigned kDequeSubarraySize>
	void DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::copyBackward(const iterator& first, const iterator& last, true_type)
	{
	...
	{
		eastl::uninitializedCopy(x.mItBegin, x.mItEnd, mItBegin);
	}
	...
	deque<T, Allocator, kDequeSubarraySize>::operator=(const this_type& x)
	{
		if(&x != this) // If not assigning to ourselves...
		{
			...
			{
				// We can't currently use setCapacity(0) or shrink_to_fit, because they 
	...
	deque<T, Allocator, kDequeSubarraySize>::begin() EASTL_NOEXCEPT
	{
		return mItBegin;
	}
	...
	deque<T, Allocator, kDequeSubarraySize>::begin() const EASTL_NOEXCEPT
	{
		return mItBegin;
	}
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Allocator, kDequeSubarraySize>::const_iterator 
	deque<T, Allocator, kDequeSubarraySize>::cbegin() const EASTL_NOEXCEPT
	{
		return mItBegin;
	}
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Allocator, kDequeSubarraySize>::iterator 
	deque<T, Allocator, kDequeSubarraySize>::end() EASTL_NOEXCEPT
	{
		return mItEnd;
	}
	...
	deque<T, Allocator, kDequeSubarraySize>::end() const EASTL_NOEXCEPT
	{
		return mItEnd;
	}
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Allocator, kDequeSubarraySize>::const_iterator
	deque<T, Allocator, kDequeSubarraySize>::cend() const EASTL_NOEXCEPT
	{
		return mItEnd;
	}
	...
	deque<T, Allocator, kDequeSubarraySize>::rbegin() EASTL_NOEXCEPT
	{
	...
	deque<T, Allocator, kDequeSubarraySize>::rbegin() const EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Allocator, kDequeSubarraySize>::const_reverse_iterator
	deque<T, Allocator, kDequeSubarraySize>::crbegin() const EASTL_NOEXCEPT
	{
		return const_reverse_iterator(mItEnd);
	}
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Allocator, kDequeSubarraySize>::reverse_iterator
	deque<T, Allocator, kDequeSubarraySize>::rend() EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Allocator, kDequeSubarraySize>::const_reverse_iterator
	deque<T, Allocator, kDequeSubarraySize>::rend() const EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Allocator, kDequeSubarraySize>::const_reverse_iterator
	deque<T, Allocator, kDequeSubarraySize>::crend() const EASTL_NOEXCEPT
	{
		return const_reverse_iterator(mItBegin);
	}
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline bool deque<T, Allocator, kDequeSubarraySize>::empty() const EASTL_NOEXCEPT
	{
	...
	inline deque<T, Allocator, kDequeSubarraySize>::size() const EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline void deque<T, Allocator, kDequeSubarraySize>::setCapacity(size_type n)
	{
		...
		// implementing setCapacity() is to do what we do below.
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Allocator, kDequeSubarraySize>::pushFront(const value_type& value)
	{
		emplace_front(value);
	}
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Allocator, kDequeSubarraySize>::reference
	deque<T, Allocator, kDequeSubarraySize>::pushFront()
	{
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Allocator, kDequeSubarraySize>::pushBack(const value_type& value)
	{
		emplace_back(value);
	}
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Allocator, kDequeSubarraySize>::reference
	deque<T, Allocator, kDequeSubarraySize>::pushBack()
	{
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Allocator, kDequeSubarraySize>::popFront()
	{
		#if EASTL_ASSERT_ENABLED
			if(EASTL_UNLIKELY((size_type)(mItEnd == mItBegin)))
				EASTL_FAIL_MSG("deque::popFront -- empty deque");
		#endif
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Allocator, kDequeSubarraySize>::popBack()
	{
		#if EASTL_ASSERT_ENABLED
			if(EASTL_UNLIKELY((size_type)(mItEnd == mItBegin)))
				EASTL_FAIL_MSG("deque::popBack -- empty deque");
		#endif
	...
	deque<T, Allocator, kDequeSubarraySize>::erase(const_iterator position)
	{
		...
		{
			itNext.copyBackward(mItBegin, itPosition, eastl::has_trivial_relocate<value_type>());
			popFront();
		}
		else
		{
	...
	deque<T, Allocator, kDequeSubarraySize>::erase(const_iterator first, const_iterator last)
	{
		...
		#if EASTL_ASSERT_ENABLED
			if(EASTL_UNLIKELY(!(validateIterator(itFirst) & isf_valid)))
				EASTL_FAIL_MSG("deque::erase -- invalid iterator");
			if(EASTL_UNLIKELY(!(validateIterator(itLast) & isf_valid)))
				EASTL_FAIL_MSG("deque::erase -- invalid iterator");
		#endif
		...
		if((itFirst != mItBegin) || (itLast != mItEnd)) // If not erasing everything... (We expect that the user won't call erase(begin, end) because instead the user would just call clear.)
		{
			...
			if(i < (difference_type)((size() - n) / 2)) // Should we move the front entries forward or the back entries backward? We divide the range in half.
			{
				...
				itLast.copyBackward(mItBegin, itFirst, eastl::has_trivial_relocate<value_type>());
	...
	void deque<T, Allocator, kDequeSubarraySize>::DoInitFromIterator(InputIterator first, InputIterator last, EASTL_ITC_NS::input_iterator_tag)
	{
		base_type::DoInit(0); // Call the base uninitialized init function, but don't actually allocate any values.
			...
			try
			{
				...
				// pushBack for each item. It can be slow because it will keep reallocating the 
				...
				{                               // Thus the standard *requires* that we do this (inefficient) implementation.  
					pushBack(*first);          // Luckily, InputIterators are in practice almost never used, so this code will likely never get executed.
				}
		#if EASTL_EXCEPTIONS_ENABLED
			}
			catch(...)
	...
	void deque<T, Allocator, kDequeSubarraySize>::DoInitFromIterator(ForwardIterator first, ForwardIterator last, EASTL_ITC_NS::forward_iterator_tag)
	{
		...
		base_type::DoInit(n); // Call the base uninitialized init function.
			...
			try
			{
				...
				{
					// We implment an algorithm here whereby we use uninitializedCopy() and advance() instead of just iterating from first to last and constructing as we go. The reason for this is that we can take advantage of POD data types and implement construction as memcpy operations.
					...
					eastl::uninitializedCopy((non_const_iterator_type)first, (non_const_iterator_type)current, (non_const_value_type*)*pPtrArrayCurrent);
					first = current;
				}

				eastl::uninitializedCopy((non_const_iterator_type)first, (non_const_iterator_type)last, (non_const_value_type*)mItEnd.mpBegin);
		#if EASTL_EXCEPTIONS_ENABLED
			}
			catch(...)
			{
	...
	void deque<T, Allocator, kDequeSubarraySize>::DoFillInit(const value_type& value)
	{
		value_type** pPtrArrayCurrent = mItBegin.mpCurrentArrayPtr;
			...
			try
			{
				...
				{
					eastl::uninitializedFill(*pPtrArrayCurrent, *pPtrArrayCurrent + kDequeSubarraySize, value);
					++pPtrArrayCurrent;
				}
				eastl::uninitializedFill(mItEnd.mpBegin, mItEnd.mpCurrent, value);
		#if EASTL_EXCEPTIONS_ENABLED
			}
			catch(...)
			{
		...
		if(position.mpCurrent == mItBegin.mpCurrent) // If inserting at the beginning or into an empty container...
		{
			iterator itNewBegin(DoReallocSubarray(n, kSideFront)); // itNewBegin to mItBegin refers to memory that isn't initialized yet; so it's not truly a valid iterator. Or at least not a dereferencable one.
				...
				try
				{
					...
					eastl::uninitializedCopy(first, last, itNewBegin);
					mItBegin = itNewBegin;
			#if EASTL_EXCEPTIONS_ENABLED
				}
				catch(...)
		...
		else if(EASTL_UNLIKELY(position.mpCurrent == mItEnd.mpCurrent)) // If inserting at the end (i.e. appending)...
		{
			const iterator itNewEnd(DoReallocSubarray(n, kSideBack)); // mItEnd to itNewEnd refers to memory that isn't initialized yet; so it's not truly a valid iterator. Or at least not a dereferencable one.
				...
				try
				{
					...
					// when inserting from a std::initializer_list, for example.
					// To do: solve this by having a template or runtime parameter which specifies move vs copy.
					eastl::uninitializedCopy(first, last, mItEnd);
					mItEnd = itNewEnd;
			#if EASTL_EXCEPTIONS_ENABLED
				}
				catch(...)
				...
				const iterator itPosition(mItBegin + nInsertionIndex); // We need to reset this value because the reallocation above can invalidate iterators.
					...
					try
					{
						...
						if(nInsertionIndex >= (difference_type)n) // If the newly inserted items will be entirely within the old area...
						{
							...
							eastl::uninitializedCopy(mItBegin, itUCopyEnd, itNewBegin); // This can throw.
						...
						else // Else the newly inserted items are going within the newly allocated area at the front.
						{
							...
							eastl::uninitializedCopyCopy(mItBegin, itPosition, first, mid, itNewBegin); // This can throw.
				...
				const iterator        itPosition(mItEnd - nPushedCount); // We need to reset this value because the reallocation above can invalidate iterators.
					...
					try
					{
						...
						if(nPushedCount > (difference_type)n)
						{
							...
							eastl::uninitializedCopy(itUCopyEnd, mItEnd, mItEnd);
							eastl::copyBackward(itPosition, itUCopyEnd, itOldEnd);
						...
						else
						{
							...
							eastl::uninitializedCopyCopy(mid, last, itPosition, mItEnd, mItEnd);
		...
		if(position.mpCurrent == mItBegin.mpCurrent) // If inserting at the beginning...
		{
			const iterator itNewBegin(DoReallocSubarray(n, kSideFront));
				...
				try
				{
					...
					eastl::uninitializedFill(itNewBegin, mItBegin, value);
					mItBegin = itNewBegin;
			#if EASTL_EXCEPTIONS_ENABLED
				}
				catch(...)
		...
		else if(EASTL_UNLIKELY(position.mpCurrent == mItEnd.mpCurrent)) // If inserting at the end (i.e. appending)...
		{
			const iterator itNewEnd(DoReallocSubarray(n, kSideBack));
				...
				try
				{
					...
					// deque, insertion at either the front or back doesn't cause a reallocation
					// or move of data in the middle. That's a key feature of deques, in fact.
					eastl::uninitializedFill(mItEnd, itNewEnd, value);
					mItEnd = itNewEnd;
			#if EASTL_EXCEPTIONS_ENABLED
				}
				catch(...)
				...
				const iterator itPosition(mItBegin + nInsertionIndex); // We need to reset this value because the reallocation above can invalidate iterators.
					...
					try
					{
						...
						if(nInsertionIndex >= (difference_type)n) // If the newly inserted items will be entirely within the old area...
						{
							...
							eastl::uninitializedMove_if_noexcept(mItBegin, itUCopyEnd, itNewBegin); // This can throw.
						...
						{
							eastl::uninitializedMove_fill(mItBegin, itPosition, itNewBegin, mItBegin, valueSaved); // This can throw.
				...
				const iterator        itPosition(mItEnd - nPushedCount); // We need to reset this value because the reallocation above can invalidate iterators.
					...
					try
					{
						...
						if(nPushedCount > (difference_type)n) // If the newly inserted items will be entirely within the old area...
						{
							...
							eastl::uninitializedMove_if_noexcept(itUCopyEnd, mItEnd, mItEnd); // This can throw.
						...
						{
							eastl::uninitializedFill_move(mItEnd, itPosition + (difference_type)n, valueSaved, itPosition, mItEnd); // This can throw.
	...
	template <typename T, typename Allocator, unsigned kDequeSubarraySize>
	inline int deque<T, Allocator, kDequeSubarraySize>::validateIterator(const_iterator i) const
	{
