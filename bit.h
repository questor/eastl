...
namespace eastl
{
	...
	// Both types To and From must be equal in size, and must be trivially copyable.
		...
		template<typename To, typename From,
			...
			>
		>
		EA_CONSTEXPR To bit_cast(const From& from) EASTL_NOEXCEPT
		{
		...
		template<typename To, typename From,
			...
			>
		>
		inline To bit_cast(const From& from) EASTL_NOEXCEPT
		{
	...
	EA_CONSTEXPR int countl_zero(const T num) EASTL_NOEXCEPT
	{
	...
	template <typename T, typename = eastl::enable_if_t<eastl::is_unsigned_v<T>>>
	EA_CONSTEXPR bool has_single_bit(const T num) EASTL_NOEXCEPT
	{
		return num != 0 && (num & (num - 1)) == 0;
	}

	template <typename T, typename = eastl::enable_if_t<eastl::is_unsigned_v<T>>>
	EA_CONSTEXPR T bit_ceil(const T num) EASTL_NOEXCEPT
	{
		if (num <= 1U)
		{
			return T(1);
	...
	template <typename T, typename = eastl::enable_if_t<eastl::is_unsigned_v<T>>>
	EA_CONSTEXPR T bit_floor(const T num) EASTL_NOEXCEPT
	{
		if (num == 0)
		{
			return T(0);
	...
	template <typename T, typename = eastl::enable_if_t<eastl::is_unsigned_v<T>>>
	EA_CONSTEXPR T bit_width(const T num) EASTL_NOEXCEPT
	{
