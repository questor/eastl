...
namespace eastl
{
	...
	/// read and written via popFront and pushBack.
	///
	/// The benefit of a ring buffer is that memory allocations don't occur
	/// and new elements are neither added nor removed from the container.
	/// Elements in the container are simply assigned values in circles around
	...
	///                                           end begin
	///
	/// A pushBack operation on a ring buffer assigns the new value to end.  
	...
	/// you can use pushBack to add items and popFront to remove them; or you can
	/// use pushFront to add items and popBack to remove them. You aren't 
	...
	/// of these containers, such as fixedString. Since ring buffer works via copying
	...
	///     }
	///     
	///     // Add a new string, using pushFront() and front() instead of 
	///     // pushFront(str) in order to avoid creating a temporary str.
	///     debugLogText.pushFront();
	...
	public:
		...
		bool                   full()     const EASTL_NOEXCEPT;
		size_type              size()     const EASTL_NOEXCEPT;
		size_type              capacity() const EASTL_NOEXCEPT;

		void                   resize(size_type n);
		void                   setCapacity(size_type n); // Sets the capacity to the given value, including values less than the current capacity. Adjusts the size downward if n < size, by throwing out the oldest elements in the buffer.
		...
		: c(allocator)
	{
		...
		if(c.getAllocator() == x.c.getAllocator())
			swap(x); // We are leaving x in an unusual state by swapping default-initialized members with it, as it won't be usable and can be only destructible.
		else
			operator=(x);
	}
	...
	ring_buffer<T, Container, Allocator>::begin() EASTL_NOEXCEPT
	{
	...
	ring_buffer<T, Container, Allocator>::begin() const EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::const_iterator
	ring_buffer<T, Container, Allocator>::cbegin() const EASTL_NOEXCEPT
	{
		return const_iterator(const_cast<Container*>(&c), mBegin); // We trust that the const_iterator will respect const-ness.
	}
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::iterator
	ring_buffer<T, Container, Allocator>::end() EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::const_iterator
	ring_buffer<T, Container, Allocator>::end() const EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::const_iterator
	ring_buffer<T, Container, Allocator>::cend() const EASTL_NOEXCEPT
	{
		return const_iterator(const_cast<Container*>(&c), mEnd); // We trust that the const_iterator will respect const-ness.
	}
	...
	ring_buffer<T, Container, Allocator>::rbegin() EASTL_NOEXCEPT
	{
	...
	ring_buffer<T, Container, Allocator>::rbegin() const EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::const_reverse_iterator
	ring_buffer<T, Container, Allocator>::crbegin() const EASTL_NOEXCEPT
	{
		return const_reverse_iterator(const_iterator(const_cast<Container*>(&c), mEnd));
	}
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::reverse_iterator
	ring_buffer<T, Container, Allocator>::rend() EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::const_reverse_iterator
	ring_buffer<T, Container, Allocator>::rend() const EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::const_reverse_iterator
	ring_buffer<T, Container, Allocator>::crend() const EASTL_NOEXCEPT
	{
		return const_reverse_iterator(const_iterator(const_cast<Container*>(&c), mBegin));
	}
	...
	template <typename T, typename Container, typename Allocator>
	bool ring_buffer<T, Container, Allocator>::empty() const EASTL_NOEXCEPT
	{
		return mBegin == mEnd;
	}
	...
	template <typename T, typename Container, typename Allocator>
	bool ring_buffer<T, Container, Allocator>::full() const EASTL_NOEXCEPT
	{
	...
	ring_buffer<T, Container, Allocator>::size() const EASTL_NOEXCEPT
	{
		return mSize;
	...
	*/
	...
	namespace Internal 
	{
		...
		template <typename T>
		struct has_overflow_allocator<T, void_t<decltype(declval<T>().getOverflowAllocator())>> : true_type {};
		...
		{
			auto& operator()(Container& c) { return c.getOverflowAllocator(); }
		};

		template <typename Container>
		struct GetFixedContainerCtorAllocator<Container, false>
		{
			auto& operator()(Container& c) { return c.getAllocator(); }
		};
	} // namespace Internal
	...
	struct ContainerTemporary<Container, true>
	{
		...
		ContainerTemporary(Container& parentContainer)
		    : mAllocator(&parentContainer.getAllocator())
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::size_type
	ring_buffer<T, Container, Allocator>::capacity() const EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Container, typename Allocator>
	void ring_buffer<T, Container, Allocator>::setCapacity(size_type n)
	{
	...
	/// being overwritten and the begin position being moved foward one position.
	template <typename T, typename Container, typename Allocator>
	void ring_buffer<T, Container, Allocator>::pushBack(const value_type& value)
	{
		*mEnd = value;
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::reference
	ring_buffer<T, Container, Allocator>::pushBack()
	{
		// We don't do the following assignment, as the value at mEnd is already constructed;
		// it is merely possibly not default-constructed. However, the spirit of pushBack
		// is that the user intends to do an assignment or data modification after the 
		// pushBack call. The user can always execute *back() = value_type() if he wants.
		//*mEnd = value_type();
	...
	template <typename T, typename Container, typename Allocator>
	void ring_buffer<T, Container, Allocator>::popBack()
	{
	...
	template <typename T, typename Container, typename Allocator>
	void ring_buffer<T, Container, Allocator>::pushFront(const value_type& value)
	{
	...
	template <typename T, typename Container, typename Allocator>
	typename ring_buffer<T, Container, Allocator>::reference
	ring_buffer<T, Container, Allocator>::pushFront()
	{
		if(EASTL_UNLIKELY(mBegin == c.begin()))
			mBegin = c.end();
		...
		// See comments above in pushBack for why we don't execute this:
		// *mBegin = value_type();
	...
	template <typename T, typename Container, typename Allocator>
	void ring_buffer<T, Container, Allocator>::popFront()
	{
	...
	ring_buffer<T, Container, Allocator>::insert(const_iterator position, const value_type& value)
	{
		...
		eastl::copyBackward(itPosition, beforeEnd, end());
		*itPosition = value;
	...
	inline bool ring_buffer<T, Container, Allocator>::validate() const
	{
		...
		if((validateIterator(begin()) & (isf_valid | isf_current)) != (isf_valid | isf_current))
			return false;

		if((validateIterator(end()) & (isf_valid | isf_current)) != (isf_valid | isf_current))
			return false;
	...
	template <typename T, typename Container, typename Allocator>
	inline int ring_buffer<T, Container, Allocator>::validateIterator(const_iterator i) const
	{
