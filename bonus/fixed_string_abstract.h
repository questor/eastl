...
namespace eastl
{
	...
	public:
		...
		fixedString(const this_type& x);                                                   // Currently we don't support overflowAllocator specification for other constructors, for simplicity.
		fixedString(const base_type& x);
		fixedString(const abstract_type& x);
		...
		fixedString substr(size_type position, size_type n) const;
		fixedString left(size_type n) const;
		fixedString right(size_type n) const;
		...
		: base_type(fixedAllocator_type(mBuffer.buffer))
	{
		...
		mpAllocFreeMethod = &fixedString::AllocFreeMethod;
	...
	template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
	inline fixedString<T, nodeCount, bEnableOverflow, OverflowAllocator>::fixedString(const abstract_type& x)
		: base_type(fixedAllocator_type(mBuffer.buffer))
	{
	...
	template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
	inline typename fixedString<T, nodeCount, bEnableOverflow, OverflowAllocator>::
	this_type& fixedString<T, nodeCount, bEnableOverflow, OverflowAllocator>::operator=(const abstract_type& x)
	{
	...
	inline void fixedString<T, nodeCount, bEnableOverflow, OverflowAllocator>::setCapacity(size_type n)
	{
		...
		template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
		inline void fixedString<T, nodeCount, bEnableOverflow, OverflowAllocator>::reset()
		{
			reset_lose_memory();
		}
	#endif
	...
	template <typename T, int nodeCount, bool bEnableOverflow, typename OverflowAllocator>
	inline void* fixedString<T, nodeCount, bEnableOverflow, OverflowAllocator>::AllocFreeMethod(size_t n, void* pBuffer, void* pContext)
	{
		// Use the basicString method to reuse the same allocator code
