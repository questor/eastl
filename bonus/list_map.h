...
namespace eastl
{
	...
	/// This is similar to eastl::useFirst, however it assumes that the input type is an object
	...
	/// 
	/// Note: Like a map, keys must still be unique.  As such, pushBack() and pushFront() operations
	...
	///     pushFront(), pushBack(), popFront() and popBack() have O(log n) operation time, similar to map::insert(), rather than O(1) time like a list,
	...
	///     The invitation times out - in main loop:  while( !listMap.empty() && listMap.front().IsExpired() ) { listMap.popFront(); }
	...
	public:
		...
		typedef typename eastl::pair<iterator, bool>                                                            insert_return_type;  // This is intentionally removed, as list_map doesn't support insert() functions, in favor of list like pushBack and pushFront
		typedef typename eastl::useFirst<value_type>                                                           extract_key;         // This is intentionally different from base_type::extract_key

		using base_type::getAllocator;
		using base_type::setAllocator;
	...
	public:
		...
		// pushFront and pushBack which takes in a key/value pair
		bool            pushFront(const value_type& value);
		bool            pushBack(const value_type& value);

		// pushFront and pushBack which take key and value separately, for convenience
		bool            pushFront(const key_type& key, const mapped_type& value);
		bool            pushBack(const key_type& key, const mapped_type& value);
	...
	public:
		...
		template <typename InputIterator>
			...
			iterator       lowerBound(const key_type& key) = delete;
			const_iterator lowerBound(const key_type& key) const = delete;

			iterator       upperBound(const key_type& key) = delete;
			const_iterator upperBound(const key_type& key) const = delete;
		 
			eastl::pair<iterator, iterator>             equalRange(const key_type& key) = delete;
			eastl::pair<const_iterator, const_iterator> equalRange(const key_type& key) const = delete;
	...
	public:
		...
		#if 0
			reference pushFront() = delete;
			void*     pushFrontUninitialized() = delete;
	...
	inline list_map_data<Value>::list_map_data(const Value& value)
	  : mValue(value)
	{
		mpNext = NULL; // GCC 4.8 is generating warnings about referencing these values in list_map::pushFront unless we 
	...
	list_map<Key, T, Compare, Allocator>::begin() EASTL_NOEXCEPT
	{
	...
	list_map<Key, T, Compare, Allocator>::begin() const EASTL_NOEXCEPT
	{
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	inline typename list_map<Key, T, Compare, Allocator>::const_iterator 
	list_map<Key, T, Compare, Allocator>::cbegin() const EASTL_NOEXCEPT
	{
		return const_iterator(mNode.mpNext);
	}
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	inline typename list_map<Key, T, Compare, Allocator>::iterator 
	list_map<Key, T, Compare, Allocator>::end() EASTL_NOEXCEPT
	{
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	inline typename list_map<Key, T, Compare, Allocator>::const_iterator 
	list_map<Key, T, Compare, Allocator>::end() const EASTL_NOEXCEPT
	{
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	inline typename list_map<Key, T, Compare, Allocator>::const_iterator 
	list_map<Key, T, Compare, Allocator>::cend() const EASTL_NOEXCEPT
	{
		return const_iterator(&mNode);
	}
	...
	list_map<Key, T, Compare, Allocator>::rbegin() EASTL_NOEXCEPT
	{
	...
	list_map<Key, T, Compare, Allocator>::rbegin() const EASTL_NOEXCEPT
	{
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	inline typename list_map<Key, T, Compare, Allocator>::const_reverse_iterator 
	list_map<Key, T, Compare, Allocator>::crbegin() const EASTL_NOEXCEPT
	{
		return const_reverse_iterator(&mNode);
	}
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	inline typename list_map<Key, T, Compare, Allocator>::reverse_iterator 
	list_map<Key, T, Compare, Allocator>::rend() EASTL_NOEXCEPT
	{
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	inline typename list_map<Key, T, Compare, Allocator>::const_reverse_iterator 
	list_map<Key, T, Compare, Allocator>::rend() const EASTL_NOEXCEPT
	{
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	inline typename list_map<Key, T, Compare, Allocator>::const_reverse_iterator 
	list_map<Key, T, Compare, Allocator>::crend() const EASTL_NOEXCEPT
	{
		return const_reverse_iterator(mNode.mpNext);
	}
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	bool list_map<Key, T, Compare, Allocator>::pushFront(const value_type& value)
	{
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	bool list_map<Key, T, Compare, Allocator>::pushBack(const value_type& value)
	{
		internal_value_type tempValue(value);
		typename base_type::insert_return_type baseReturn = base_type::insert(tempValue);
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	bool list_map<Key, T, Compare, Allocator>::pushFront(const key_type& key, const mapped_type& value)
	{
		return pushFront(eastl::makePair(key, value));
	}

	template <typename Key, typename T, typename Compare, typename Allocator>
	bool list_map<Key, T, Compare, Allocator>::pushBack(const key_type& key, const mapped_type& value)
	{
		return pushBack(eastl::makePair(key, value));
	}

	template <typename Key, typename T, typename Compare, typename Allocator>
	void list_map<Key, T, Compare, Allocator>::popFront()
	{
		#if EASTL_ASSERT_ENABLED
			if (EASTL_UNLIKELY(empty()))
				EASTL_FAIL_MSG("list_map::popFront -- empty container");
		#endif
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	void list_map<Key, T, Compare, Allocator>::popBack()
	{
		#if EASTL_ASSERT_ENABLED
			if (EASTL_UNLIKELY(empty()))
				EASTL_FAIL_MSG("list_map::popBack -- empty container");
		#endif
	...
	template <typename Key, typename T, typename Compare, typename Allocator>
	int list_map<Key, T, Compare, Allocator>::validateIterator(const_iterator iter) const
	{
