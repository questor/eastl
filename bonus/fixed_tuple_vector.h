...
namespace eastl
{
	...
	///
		...
		#define EASTL_FIXED_TUPLE_VECTOR_DEFAULT_NAME EASTL_DEFAULT_NAME_PREFIX " fixed_tuple_vector" // Unless the user overrides something, this is "EASTL fixedVector".
	#endif
...
class fixed_tuple_vector : public TupleVecInternal::TupleVecImpl<fixedVector_allocator<
...
{
public:
	typedef fixedVector_allocator<
		TupleVecInternal::TupleRecurser<Ts...>::GetTotalAllocationSize(nodeCount, 0), 1,
		TupleVecInternal::TupleRecurser<Ts...>::GetTotalAlignment(), 0,
		bEnableOverflow, EASTLAllocatorType> fixedAllocator_type;
	typedef aligned_buffer<fixedAllocator_type::kNodesSize, fixedAllocator_type::kNodeAlignment> aligned_buffer_type;
	typedef fixed_tuple_vector<nodeCount, bEnableOverflow, Ts...> this_type;
	typedef EASTLAllocatorType overflow_allocator_type;

	typedef TupleVecInternal::TupleVecImpl<fixedAllocator_type, make_index_sequence<sizeof...(Ts)>, Ts...> base_type;
...
public:
	fixed_tuple_vector()
		: base_type(fixedAllocator_type(mBuffer.buffer), mBuffer.buffer, nodeCount, fixedAllocator_type::kNodeSize)
	{ }

	fixed_tuple_vector(const overflow_allocator_type& allocator)
		: base_type(fixedAllocator_type(mBuffer.buffer, allocator), mBuffer.buffer, nodeCount, fixedAllocator_type::kNodeSize)
	{ }

	fixed_tuple_vector(this_type&& x)
		: base_type(fixedAllocator_type(mBuffer.buffer), mBuffer.buffer, nodeCount, fixedAllocator_type::kNodeSize)
	{ 
		base_type::getAllocator().copy_overflow_allocator(x.getAllocator());
	...
	size_type maxSize() const { return nodeCount; }
	...
	bool hasOverflowed() const { return ((void*)base_type::mpData != (void*)mBuffer.buffer); }
	// Returns the value of the bEnableOverflow template parameter.
	bool can_overflow() const { return bEnableOverflow; }

	const overflow_allocator_type& getOverflowAllocator() const { return base_type::getAllocator().getOverflowAllocator(); }
};
