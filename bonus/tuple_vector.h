...
// inserts, erases, pushBacks, and random-access. It also provides a
// RandomAccessIterator and corresponding functionality, making it compatible
// with most STL (and STL-esque) algorithms such as ranged-for loops, findIf,
// removeIf, or sort.
...
struct TupleRecurser<>
{
	...
	static pair<void*, size_type> DoAllocate(TupleVecImpl<Allocator, Indices, VecTypes...> &vec, void** ppNewLeaf, size_type capacity, size_type offset)
	{
		...
		void* ptr = capacity ? allocate_memory(vec.getAllocator(), offset, alignment, 0) : nullptr;
...
struct TupleVecLeaf
{
	...
	void DoUninitializedMoveAndDestruct(const size_type begin, const size_type end, T* pDest)
	{
		...
		T* pEnd = mpData + end;
		eastl::uninitializedMove_ptr_if_noexcept(pBegin, pEnd, pDest);
	...
	void DoInsertAndFill(size_type pos, size_type n, size_type numElements, const T& arg)
	{
		...
		{
			eastl::uninitializedMove_ptr(pDataEnd - n, pDataEnd, pDataEnd);
		...
		{
			eastl::uninitializedFillNPtr(pDataEnd, n - nExtra, temp);
			eastl::uninitializedMove_ptr(pDest, pDataEnd, pDataEnd + n - nExtra);
	...
	void DoInsertRange(T* pSrcBegin, T* pSrcEnd, T* pDestBegin, size_type numDataElements)
	{
		...
		{
			eastl::uninitializedCopy(pSrcEnd - (n - nExtra), pSrcEnd, pDataEnd);
			eastl::uninitializedMove_ptr(pDestBegin, pDataEnd, pDataEnd + n - nExtra);
	...
	void DoInsertValue(size_type pos, size_type numElements, T&& arg)
	{
		...
		eastl::uninitializedMove_ptr(pDataEnd - 1, pDataEnd, pDataEnd);
...
public:
	...
	TupleVecImpl(this_type&& x)
		: mDataSizeAndAllocator(0, eastl::move(x.getAllocator()))
	{
		swap(x);
	}
	...
	TupleVecImpl(this_type&& x, const Allocator& allocator) 
		: mDataSizeAndAllocator(0, allocator)
	{
		if (getAllocator() == x.getAllocator()) // If allocators are equivalent, then we can safely swap member-by-member
		{
			swap(x);
		}
		else
	...
	TupleVecImpl(const this_type& x) 
		: mDataSizeAndAllocator(0, x.getAllocator())
	{
...
public:
	...
	{ 
		...
		if (mpData)
			EASTLFree(getAllocator(), mpData, internalDataSize()); 
	}
	...
	void assign(size_type n, const Ts&... args)
	{
		if (n > mNumCapacity)
		{
			this_type temp(n, args..., getAllocator()); // We have little choice but to reallocate with new memory.
			swap(temp);
		}
		else if (n > mNumElements) // If n > mNumElements ...
		{
			...
			swallow((eastl::uninitializedFillPtr(TupleVecLeaf<Indices, Ts>::mpData + oldNumElements,
	...
	{
#if EASTL_ASSERT_ENABLED
		if (EASTL_UNLIKELY(!validateIterator_pair(first, last)))
		...
		{
			this_type temp(first, last, getAllocator());
			swap(temp);
		}
		else
		{
			...
			if (newNumElements > mNumElements) // If n > mNumElements ...
			{
				...
				swallow((eastl::uninitializedCopyPtr((Ts*)(ppOtherData[Indices]) + firstIdx + oldNumElements,
	...
	{
#if EASTL_ASSERT_ENABLED
		if (EASTL_UNLIKELY(validateIterator(pos) == isf_none))
	...
	iterator insert(const_iterator pos, size_type n, const Ts&... args)
	{
		...
		if (newNumElements > oldNumCapacity || firstIdx != oldNumElements)
		{
			if (newNumElements > oldNumCapacity)
			{
				...
				swallow((eastl::uninitializedFillPtr((Ts*)ppNewLeaf[Indices] + firstIdx, (Ts*)ppNewLeaf[Indices] + lastIdx, args), 0)...);
				swallow(TupleVecLeaf<Indices, Ts>::mpData = (Ts*)ppNewLeaf[Indices]...);
	...
	iterator insert(const_iterator pos, const_iterator first, const_iterator last)
	{
		...
		if (newNumElements > oldNumCapacity || posIdx != oldNumElements)
		{
			if (newNumElements > oldNumCapacity)
			{
				...
				swallow((eastl::uninitializedCopyPtr((Ts*)(ppOtherData[Indices]) + firstIdx,
						                       (Ts*)(ppOtherData[Indices]) + lastIdx,
	...
	iterator insert(const_iterator pos, const value_tuple* first, const value_tuple* last)
	{
		...
		if (newNumElements > oldNumCapacity || posIdx != oldNumElements)
		{
			...
			else
			{
				...
				{
					swallow((eastl::uninitializedMove_ptr((Ts*)ppDataEnd[Indices] - numToInsert,
				...
				else
				{
					...
					swallow((eastl::uninitializedMove_ptr((Ts*)ppDataBegin[Indices],
	...
	{
#if EASTL_ASSERT_ENABLED
		if (EASTL_UNLIKELY(validateIterator(first) == isf_none || validateIterator(last) == isf_none))
	...
	void resize(size_type n)
	{
		...
		if (n > oldNumElements)
		{
			...
			swallow((eastl::uninitialized_default_fillN(TupleVecLeaf<Indices, Ts>::mpData + oldNumElements, n - oldNumElements), 0)...);
		}
		else
		{
	...
	void shrink_to_fit()
	{
		this_type temp(move_iterator<iterator>(begin()), move_iterator<iterator>(end()), getAllocator());
		swap(temp);
	}
	...
	void popBack()
	{
		...
		if (EASTL_UNLIKELY(mNumElements <= 0))
			EASTL_FAIL_MSG("tuple_vector::popBack -- container is empty");
#endif
		size_type oldNumElements = mNumElements--;
	...
	void swap(this_type& x)
	{
		...
		eastl::swap(getAllocator(), x.getAllocator());
	...
	void pushBack(Ts&&... args) { emplace_back(eastl::forward<Ts>(args)...); }
	void pushBack(const_reference_tuple tup) { pushBack(eastl::get<Indices>(tup)...); }
	void pushBack(rvalue_tuple tup) { emplace_back(eastl::forward<Ts>(eastl::get<Indices>(tup))...); }
	...
	bool empty() const EASTL_NOEXCEPT { return mNumElements == 0; }
	size_type size() const EASTL_NOEXCEPT { return mNumElements; }
	size_type capacity() const EASTL_NOEXCEPT { return mNumCapacity; }

	iterator begin() EASTL_NOEXCEPT { return iterator(this, 0); }
	const_iterator begin() const EASTL_NOEXCEPT { return const_iterator((const_this_type*)(this), 0); }
	const_iterator cbegin() const EASTL_NOEXCEPT { return const_iterator((const_this_type*)(this), 0); }

	iterator end() EASTL_NOEXCEPT { return iterator(this, size()); }
	const_iterator end() const EASTL_NOEXCEPT { return const_iterator((const_this_type*)(this), size()); }
	const_iterator cend() const EASTL_NOEXCEPT { return const_iterator((const_this_type*)(this), size()); }

	reverse_iterator rbegin() EASTL_NOEXCEPT { return reverse_iterator(end()); }
	const_reverse_iterator rbegin() const  EASTL_NOEXCEPT { return const_reverse_iterator(end()); }
	const_reverse_iterator crbegin() const EASTL_NOEXCEPT { return const_reverse_iterator(end()); }
	
	reverse_iterator rend() EASTL_NOEXCEPT { return reverse_iterator(begin()); }
	const_reverse_iterator rend() const EASTL_NOEXCEPT { return const_reverse_iterator(begin()); }
	const_reverse_iterator crend() const EASTL_NOEXCEPT { return const_reverse_iterator(begin()); }

	ptr_tuple data() EASTL_NOEXCEPT { return ptr_tuple(TupleVecLeaf<Indices, Ts>::mpData...); }
	const_ptr_tuple data() const EASTL_NOEXCEPT { return const_ptr_tuple(TupleVecLeaf<Indices, Ts>::mpData...); }
	...
	int validateIterator(const_iterator iter) const EASTL_NOEXCEPT
	{
	...
	static bool validateIterator_pair(const_iterator first, const_iterator last) EASTL_NOEXCEPT
	{
	...
	int validateIterator(Iterator iter) const EASTL_NOEXCEPT { return validateIterator(unwrap_iterator(iter)); }

	template <typename Iterator, typename = typename enable_if<is_iterator_wrapper<Iterator>::value, bool>::type>
	static bool validateIterator_pair(Iterator first, Iterator last) EASTL_NOEXCEPT { return validateIterator_pair(unwrap_iterator(first), unwrap_iterator(last)); }

	allocator_type& getAllocator() EASTL_NOEXCEPT { return mDataSizeAndAllocator.second(); }
	const allocator_type& getAllocator() const EASTL_NOEXCEPT { return mDataSizeAndAllocator.second(); }

	void setAllocator(const allocator_type& alloc) { mDataSizeAndAllocator.second() = alloc; }
...
protected:
	...
	size_type& internalDataSize() EASTL_NOEXCEPT { return mDataSizeAndAllocator.first(); }
	size_type const& internalDataSize() const EASTL_NOEXCEPT { return mDataSizeAndAllocator.first(); }
	...
	{
#if EASTL_ASSERT_ENABLED
		if (EASTL_UNLIKELY(!validateIterator_pair(begin, end)))
			EASTL_FAIL_MSG("tuple_vector::erase -- invalid iterator pair");
#endif
		...
		mNumElements = newNumElements;
		swallow((eastl::uninitializedMove_ptr(eastl::move_iterator<Ts*>((Ts*)(ppOtherData[Indices]) + beginIdx),
	...
	void DoInitFromIterator(const_iterator begin, const_iterator end)
	{
		...
		DoConditionalReallocate(0, mNumCapacity, newNumElements);
		mNumElements = newNumElements;
		swallow((eastl::uninitializedCopyPtr((Ts*)(ppOtherData[Indices]) + beginIdx,
	...
	void DoInitFillArgs(size_type n, const Ts&... args)
	{
		...
		mNumElements = n;
		swallow((eastl::uninitializedFillPtr(TupleVecLeaf<Indices, Ts>::mpData, TupleVecLeaf<Indices, Ts>::mpData + n, args), 0)...);
	}
	...
	void DoInitDefaultFill(size_type n)
	{
		DoConditionalReallocate(0, mNumCapacity, n);
		mNumElements = n;
		swallow((eastl::uninitialized_default_fillN(TupleVecLeaf<Indices, Ts>::mpData, n), 0)...);
	}
