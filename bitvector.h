...
/*
	...
	template <bool value>
	typename bitvector<Allocator, Element, Container>::iterator 
	bitvector<Allocator, Element, Container>::findFirst()
	{
		return begin();
	}

	template <bool value> iterator findNext(const_iterator it);
	template <bool value> iterator findLast();
	template <bool value> iterator findPrev(const_iterator it);
						
	template <bool value> const_iterator findFirst() const;
	template <bool value> const_iterator findNext(const_iterator it) const;
	template <bool value> const_iterator findLast() const;
	template <bool value> const_iterator findPrev(const_iterator it) const;
*/
	...
	bitvector<Allocator, Element, Container>::data() EASTL_NOEXCEPT
	{
	...
	bitvector<Allocator, Element, Container>::data() const EASTL_NOEXCEPT
	{
		return mContainer.data();
	}
	...
	bitvector<Allocator, Element, Container>::insert(const_iterator position, value_type value)
	{
		iterator iPosition(position.get_reference_type()); // This is just a non-const version of position.
			...
			if(EASTL_UNLIKELY(validateIterator(iPosition) & eastl::isf_valid) == 0)
	...
	*/
	...
	bitvector<Allocator, Element, Container>::erase(const_iterator position)
	{
		...
		#if EASTL_ASSERT_ENABLED
			if(EASTL_UNLIKELY(validateIterator(iPosition) & eastl::isf_can_dereference) == 0)
		...
		EASTL_ASSERT(validateIterator(iPosition) & eastl::isf_valid);
		return iPosition;
	}
	...
	bitvector<Allocator, Element, Container>::erase(const_iterator first, const_iterator last)
	{
		...
		#if EASTL_ASSERT_ENABLED
			if(EASTL_UNLIKELY(validateIterator(iLast) & eastl::isf_valid) == 0)
				EASTL_FAIL_MSG("bitvector::erase -- invalid iterator");
		#endif
		...
		{
			#if EASTL_ASSERT_ENABLED
				if(EASTL_UNLIKELY(validateIterator(iFirst) & eastl::isf_can_dereference) == 0)
					EASTL_FAIL_MSG("bitvector::erase -- invalid iterator");
			#endif
			...
			// Verify that no reallocation occurred.
			#if EASTL_ASSERT_ENABLED
				if(EASTL_UNLIKELY(validateIterator(iFirst) & eastl::isf_valid) == 0)
					EASTL_FAIL_MSG("bitvector::erase -- invalid iterator");
			#endif
		}
