...
namespace eastl
{
	...
	public:
	  ...
	  //unsigned long toUlong()  const;    // We inherit this from the base class.
	  //uint32_t      to_uint32() const;
	  //uint64_t      to_uint64() const;
	...
	*/
	...
	template <size_t NW, typename WordType>
	inline unsigned long BitsetBase<NW, WordType>::toUlong() const
	{
		#if EASTL_EXCEPTIONS_ENABLED
			for(size_t i = 1; i < NW; ++i)
			{
				if(mWord[i])
					throw std::overflow_error("BitsetBase::toUlong");
			}
		#endif
...
EA_RESTORE_GCC_WARNING()
	...
	*/
	...
	template <typename WordType>
	inline unsigned long BitsetBase<1, WordType>::toUlong() const
	{
		#if EASTL_EXCEPTIONS_ENABLED
				...
				// Verify that high bits are not set and thus that toUlong doesn't lose information.
				if(mWord[0] >> 32)
					throw std::overflow_error("BitsetBase::toUlong");
			#endif
		#endif
	...
	*/
	...
	template <typename WordType>
	inline unsigned long BitsetBase<2, WordType>::toUlong() const
	{
		#if EASTL_EXCEPTIONS_ENABLED
			if(mWord[1])
				throw std::overflow_error("BitsetBase::toUlong");
	...
	*/
	...
	// template <size_t N, typename WordType>
	// inline unsigned long bitset<N, WordType>::toUlong() const
	// {
	//     return base_type::toUlong();
	// }
	...
	template <size_t N, typename WordType>
	inline typename bitset<N, WordType>::size_type
	bitset<N, WordType>::findFirst() const
	{
	...
	template <size_t N, typename WordType>
	inline typename bitset<N, WordType>::size_type
	bitset<N, WordType>::findNext(size_type last_find) const
	{
	...
	template <size_t N, typename WordType>
	inline typename bitset<N, WordType>::size_type
	bitset<N, WordType>::findLast() const
	{
	...
	template <size_t N, typename WordType>
	inline typename bitset<N, WordType>::size_type
	bitset<N, WordType>::findPrev(size_type last_find) const
	{
