...
namespace eastl
{
	...
	public:
		...
		inline ListIterator(const iterator& x) EASTL_NOEXCEPT
			: mpNode(x.mpNode)
		{
			// Empty
		}
	...
	template <typename T, typename Allocator>
	inline bool list<T, Allocator>::empty() const EASTL_NOEXCEPT
	{
		#if EASTL_LIST_SIZE_CACHE
			return (mSize == 0);
		#else
	...
	list<T, Allocator>::size() const EASTL_NOEXCEPT
	{
		#if EASTL_LIST_SIZE_CACHE
			return mSize;
		#else
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::clear() EASTL_NOEXCEPT
	{
		DoClear();
		DoInit();
		#if EASTL_LIST_SIZE_CACHE
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::reset_lose_memory() EASTL_NOEXCEPT
	{
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::pushFront(const value_type& value)
	{
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::pushFront(value_type&& value)
	{
	...
	template <typename T, typename Allocator>
	inline void* list<T, Allocator>::pushFrontUninitialized()
	{
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::pushBack(const value_type& value)
	{
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::pushBack(value_type&& value)
	{
	...
	template <typename T, typename Allocator>
	inline void* list<T, Allocator>::pushBackUninitialized()
	{
		node_type* const pNode = DoAllocateNode();
		pNode->insert(&internalNode());
		#if EASTL_LIST_SIZE_CACHE
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::popBack()
	{
		#if EASTL_ASSERT_ENABLED
			if(EASTL_UNLIKELY(static_cast<node_type*>(internalNode().mpNext) == &internalNode()))
				EASTL_FAIL_MSG("list::popBack -- empty container");
		#endif
	...
	template <typename T, typename Allocator>
	template <typename Predicate>
	inline typename list<T, Allocator>::size_type list<T, Allocator>::removeIf(Predicate predicate)
	{
		size_type numRemoved = 0;
	...
	template <typename T, typename Allocator>
	inline void list<T, Allocator>::reverse() EASTL_NOEXCEPT
	{
	...
	list<T, Allocator>::DoSort(iterator i1, iterator end2, size_type n, Compare& compare)
	{
		...
		// but that was incompatible with fixedList because the sizes could be too big.
	...
	template <typename T, typename Allocator>
	inline int list<T, Allocator>::validateIterator(const_iterator i) const
	{
