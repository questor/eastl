...
namespace eastl
{
	...
	EA_CPP14_CONSTEXPR T* data(T(&array)[N]) EASTL_NOEXCEPT
		{ return array; }

	template <class E>
	EA_CPP14_CONSTEXPR const E* data(std::initializer_list<E> il) EASTL_NOEXCEPT
		{ return il.begin(); }
	...
	template <class T, size_t N>
	EA_CPP14_CONSTEXPR size_t size(const T (&)[N]) EASTL_NOEXCEPT
		{ return N; }
	...
	EA_CPP14_CONSTEXPR ptrdiff_t ssize(const T(&)[N]) EASTL_NOEXCEPT
		{ return N; }
	...
	template <class T, size_t N>
	EA_CPP14_CONSTEXPR bool empty(const T (&)[N]) EASTL_NOEXCEPT
		{ return false; }

	template <class E>
	EA_CPP14_CONSTEXPR bool empty(std::initializer_list<E> il) EASTL_NOEXCEPT
		{ return il.size() == 0; }
	...
	//
		...
		EA_CPP14_CONSTEXPR inline T* begin(T (&arrayObject)[arraySize]) EASTL_NOEXCEPT
		{
			return arrayObject;
		}
		...
		template<typename T, size_t arraySize>
		EA_CPP14_CONSTEXPR inline T* end(T (&arrayObject)[arraySize]) EASTL_NOEXCEPT
		{
