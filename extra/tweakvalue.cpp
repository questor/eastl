...
namespace tweakvalue {
   ...
   typedef eastl::vector<TweakableSourceFile> TweakFileList;
   static TweakFileList g_tweakableFile;

   // Stored tweak values
   typedef std::pair< eastl::string, size_t> TweakValKey;
   typedef eastl::map< TweakValKey, Tweakable*> TweakValMap;
         ...
         while ((ch=strstr( ch, "_T" "V(")) ) {
            ...
            if (chend) {
               ...
               if (tv) {
                  ...
                  } else if (tv->type == Tweakable::Type_DOUBLE) {
                     tv->val.d_val = atof( strval );
                  }
               }
            } else {
...
double _TweakValue( const char *file, size_t counter, double origVal ) {
   Tweakable *tv = LookupTweakValue( file, counter );
   if (tv) {
      return tv->val.d_val;
   } else {
      Tweakable *twk = new Tweakable();
      twk->type = Tweakable::Type_DOUBLE;
      twk->val.d_val = origVal;
      return AddTweakableValue( file, counter, twk )->val.d_val;
   }
}
