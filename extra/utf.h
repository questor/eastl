...
// taken from https://github.com/jalfd/utf.hpp
...
/* Sample:
    ...
    int main() {
        ...
        utf::makeStringview(s.begin(), s.end());
        ...
        auto sv2 = utf::makeStringview(str2, str2 + 11);
...
#ifndef EASTL_EXTRA_UTF_HPP
#define EASTL_EXTRA_UTF_HPP
...
namespace utf {
    ...
    stringview<const T*> makeStringview(T (&arr)[N]) {
        return stringview<const T*>(arr, arr + N);
    }

    template <typename Iter>
    stringview<Iter>
    makeStringview(Iter first, Iter last) {
...
namespace eastl {
    void convertUtf8ToUtf16(const eastl::string &source, eastl::string16 &dest) {
        utf::stringview<const char*> sv(source.begin(), source.end());
        dest.reserve(source.length());      //likely overallocate
        sv.to<utf::utf16>(dest.begin());
    }
    void convertUtf8ToUtf16(const char* source, eastl::string16 &dest) {
        utf::stringview<const char*> sv(source, source+strlen(source));
        dest.reserve(strlen(source));             //likely overallocate
        sv.to<utf::utf16>(dest.begin());
    }
    eastl::string16 convertUtf8ToUtf16(const eastl::string &source) {
        utf::stringview<const char*> sv(source.begin(), source.end());
        eastl::string16 dest(eastl::string16::CtorDoNotInitialize{}, source.length());        //likely overallocate
        sv.to<utf::utf16>(dest.begin());
        return dest;
    }
    eastl::string16 convertUtf8ToUtf16(const char* source) {
        utf::stringview<const char*> sv(source, source+strlen(source));
        eastl::string16 dest(eastl::string16::CtorDoNotInitialize{}, strlen(source));      //likely overallocate
        sv.to<utf::utf16>(dest.begin());
        return dest;
    }

    void convertUtf16ToUtf8(const eastl::string16 &source, eastl::string &dest) {
        utf::stringview<eastl::string16::const_iterator> sv(source.begin(), source.end());
        dest.reserve(sv.codeunits<utf::utf8>()*3/2);  //estimation
        sv.to<utf::utf8>(dest.begin());
    }
    eastl::string convertUtf16ToUtf8(const eastl::string16 &source) {
        utf::stringview<eastl::string16::const_iterator> sv(source.begin(), source.end());
        eastl::string dest(eastl::string::CtorDoNotInitialize{}, sv.codeunits<utf::utf8>()*3/2);     //estimation
        sv.to<utf::utf8>(dest.begin());
        return dest;
    }
}
...
#endif  //#ifndef EASTL_EXTRA_UTF_HPP
