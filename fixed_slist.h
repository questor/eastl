...
namespace eastl
{
	...
	public:
		typedef fixed_node_allocator<sizeof(typename slist<T>::node_type), nodeCount,
					 EASTL_ALIGN_OF(typename slist<T>::node_type), 0, bEnableOverflow, OverflowAllocator> fixedAllocator_type;
		...
		: base_type(fixedAllocator_type(mBuffer))
	{
			...
			internalAllocator().setName(EASTL_FIXED_SLIST_DEFAULT_NAME);
		#endif
	}
