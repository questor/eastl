...
namespace eastl
{
	...
	inline vector<T, Allocator>::vector(size_type n, const value_type& value, const allocator_type& allocator)
		: base_type(n, allocator)
	{
		eastl::uninitializedFillN(mpBegin, n, value);
		mpEnd = mpBegin + n;
	}
	...
	{
		mpEnd = eastl::uninitializedCopy(x.mpBegin, x.mpEnd, mpBegin);
	}
	...
	template <typename T, typename Allocator>
	inline void vector<T, Allocator>::pushBack(const value_type& value)
	{
	...
	template <typename T, typename Allocator>
	inline void* vector<T, Allocator>::pushBackUninitialized()
	{
		if(mpEnd == internalCapacityPtr())
		{
	...
	inline void vector<T, Allocator>::popBack()
	{
			...
			if(EASTL_UNLIKELY(mpEnd <= mpBegin))
				EASTL_FAIL_MSG("vector::popBack -- empty vector");
		#endif
	...
	template <typename T, typename Allocator>
	inline void vector<T, Allocator>::clear() EASTL_NOEXCEPT
	{
		eastl::destruct(mpBegin, mpEnd);
		mpEnd = mpBegin;
	}
	...
	vector<T, Allocator>::DoRealloc(size_type n, ForwardIterator first, ForwardIterator last, should_copy_tag)
	{
		...
		eastl::uninitializedCopy(first, last, p); // copy-constructs p from [first,last).
		return p;
	}
	...
	{
		T* const p = DoAllocate(n); // p is of type T* but is not constructed. 
		eastl::uninitializedMove_if_noexcept(first, last, p); // move-constructs p from [first,last).
		return p;
	}
	...
	{
		// To do: Use emplace_back instead of pushBack(). Our emplace_back will work below without any ifdefs.
		for(; first != last; ++first)  // InputIterators by definition actually only allow you to iterate through them once.
			pushBack(*first);        // Thus the standard *requires* that we do this (inefficient) implementation.
	...
	inline void vector<T, Allocator>::DoInitFromIterator(ForwardIterator first, ForwardIterator last, EASTL_ITC_NS::forward_iterator_tag)
	{
		...
		eastl::uninitializedCopy(first, last, mpBegin);
	}
	...
	void vector<T, Allocator>::DoAssignValues(size_type n, const value_type& value)
	{
		...
		else if(n > size_type(mpEnd - mpBegin)) // If n > size ...
		{
			...
			eastl::uninitializedFillN(mpEnd, n - size_type(mpEnd - mpBegin), value);
			mpEnd += n - size_type(mpEnd - mpBegin);
		}
		else // else 0 <= n <= size
		{
			eastl::fillN(mpBegin, n, value);
	...
	void vector<T, Allocator>::DoAssignFromIterator(RandomAccessIterator first, RandomAccessIterator last, EASTL_ITC_NS::random_access_iterator_tag)
	{
		...
		{
			pointer const pNewEnd = eastl::copy(first, last, mpBegin); // Since we are copying to mpBegin, we don't have to worry about needing copyBackward or a memmove-like copy (as opposed to memcpy-like copy).
		...
		else // else size < n <= capacity
		{
			...
			eastl::copy(first, position, mpBegin); // Since we are copying to mpBegin, we don't have to worry about needing copyBackward or a memmove-like copy (as opposed to memcpy-like copy).
			mpEnd = eastl::uninitializedCopy(position, last, mpEnd);
		}
	}
	...
	void vector<T, Allocator>::DoInsertFromIterator(const_iterator position, BidirectionalIterator first, BidirectionalIterator last, EASTL_ITC_NS::bidirectional_iterator_tag)
	{
		...
		if(first != last)
		{
			...
			if(n <= size_type(internalCapacityPtr() - mpEnd)) // If n fits within the existing capacity...
			{
				...
				{
					eastl::uninitializedMove(mpEnd - n, mpEnd, mpEnd);
				...
				else
				{
					...
					eastl::uninitializedCopy(iTemp, last, mpEnd);
					eastl::uninitializedMove(destPosition, mpEnd, mpEnd + n - nExtra);
					eastl::copyBackward(first, iTemp, destPosition + nExtra);
				}
			...
			else // else we need to expand our capacity.
			{
				...
				pointer const   pNewData  = DoAllocate(nNewSize);
					...
					try
					{
						pNewEnd = eastl::uninitializedMove_if_noexcept(mpBegin, destPosition, pNewData);
						pNewEnd = eastl::uninitializedCopy(first, last, pNewEnd);
						pNewEnd = eastl::uninitializedMove_if_noexcept(destPosition, mpEnd, pNewEnd);
					}
					catch(...)
					{
				...
				#else
					pointer pNewEnd = eastl::uninitializedMove(mpBegin, destPosition, pNewData);
					pNewEnd         = eastl::uninitializedCopy(first, last, pNewEnd);
					pNewEnd         = eastl::uninitializedMove(destPosition, mpEnd, pNewEnd);
				#endif
	...
	void vector<T, Allocator>::DoInsertValues(const_iterator position, size_type n, const value_type& value)
	{
		...
		if(n <= size_type(internalCapacityPtr() - mpEnd)) // If n is <= capacity...
		{
			if(n > 0) // To do: See if there is a way we can eliminate this 'if' statement.
			{
				...
				{
					eastl::uninitializedFillN(mpEnd, n - nExtra, temp);
					eastl::uninitializedMove(destPosition, mpEnd, mpEnd + n - nExtra);
		...
		else // else n > capacity
		{
			...
			pointer const pNewData    = DoAllocate(nNewSize);
				...
				{
					pNewEnd = eastl::uninitializedMove_if_noexcept(mpBegin, destPosition, pNewData);
					eastl::uninitializedFillN(pNewEnd, n, value);
					pNewEnd = eastl::uninitializedMove_if_noexcept(destPosition, mpEnd, pNewEnd + n);
				}
				catch(...)
				{
					eastl::destruct(pNewData, pNewEnd);
				...
				pointer pNewEnd = eastl::uninitializedMove(mpBegin, destPosition, pNewData);
				eastl::uninitializedFillN(pNewEnd, n, value);
				pNewEnd = eastl::uninitializedMove(destPosition, mpEnd, pNewEnd + n);
			#endif
	...
	template <typename T, typename Allocator>
	void vector<T, Allocator>::DoClearCapacity() // This function exists because setCapacity() currently indirectly requires value_type to be default-constructible, 
	...
	void vector<T, Allocator>::DoGrow(size_type n)
	{
		...
		pointer pNewEnd = eastl::uninitializedMove_if_noexcept(mpBegin, mpEnd, pNewData);
	...
	void vector<T, Allocator>::DoInsertValuesEnd(size_type n, const value_type& value)
	{
		if(n > size_type(internalCapacityPtr() - mpEnd))
		{
			...
			pointer const pNewData = DoAllocate(nNewSize);
				...
				try
				{
					pNewEnd = eastl::uninitializedMove_if_noexcept(mpBegin, mpEnd, pNewData);
				}
				catch(...)
				{
					eastl::destruct(pNewData, pNewEnd);
			...
			#else
				pointer pNewEnd = eastl::uninitializedMove(mpBegin, mpEnd, pNewData);
			#endif
		...
		{
			eastl::uninitializedFillN(mpEnd, n, value);
			mpEnd += n;
		}
	}
	...
	void vector<T, Allocator>::DoInsertValuesEnd(size_type n)
	{
		if (n > size_type(internalCapacityPtr() - mpEnd))
		{
			...
			#if EASTL_EXCEPTIONS_ENABLED
				pointer pNewEnd = pNewData;  // Assign pNewEnd a value here in case the copy throws.
				try { pNewEnd = eastl::uninitializedMove_if_noexcept(mpBegin, mpEnd, pNewData); }
				catch (...)
				{
					eastl::destruct(pNewData, pNewEnd);
					DoFree(pNewData, nNewSize);
		...
		else // else (size == capacity)
		{
			...
			pointer const   pNewData  = DoAllocate(nNewSize);
				...
				{   // To do: We are not handling exceptions properly below.  In particular we don't want to 
					...
					pNewEnd = eastl::uninitializedMove_if_noexcept(mpBegin, destPosition, pNewData);           // the value first, because it might possibly be a reference to the old data being moved.
					pNewEnd = eastl::uninitializedMove_if_noexcept(destPosition, mpEnd, ++pNewEnd);
				}
				catch(...)
				{
			...
			#else
				::new((void*)(pNewData + nPosSize)) value_type(eastl::forward<Args>(args)...);                  // Because the old data is potentially being moved rather than copied, we need to move 
				pointer pNewEnd = eastl::uninitializedMove(mpBegin, destPosition, pNewData);					// the value first, because it might possibly be a reference to the old data being moved.
				pNewEnd = eastl::uninitializedMove(destPosition, mpEnd, ++pNewEnd);
			#endif
	...
	void vector<T, Allocator>::DoInsertValueEnd(Args&&... args)
	{
		...
		pointer const   pNewData  = DoAllocate(nNewSize);
			...
			pointer pNewEnd = eastl::uninitializedMove(mpBegin, mpEnd, pNewData);			//and here copy the rest of the elements into the new buffer
			pNewEnd++;
		#endif
	...
	template <typename T, typename Allocator>
	inline bool vector<T, Allocator>::validate() const EASTL_NOEXCEPT
	{
		if(mpEnd < mpBegin)
			return false;
	...
	template <typename T, typename Allocator>
	inline int vector<T, Allocator>::validateIterator(const_iterator i) const EASTL_NOEXCEPT
	{
		if(i >= mpBegin)
		{
			if(i < mpEnd)
