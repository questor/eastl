...
namespace eastl
{
	...
	vector<T, Allocator>::DoRealloc(size_type n, ForwardIterator first, ForwardIterator last, should_copy_tag)
	{
		...
		eastl::uninitializedCopyPtr(first, last, p); // copy-constructs p from [first,last).
		return p;
	}
	...
	{
		T* const p = DoAllocate(n); // p is of type T* but is not constructed. 
		eastl::uninitializedMove_ptr_if_noexcept(first, last, p); // move-constructs p from [first,last).
		return p;
	}
	...
	void vector<T, Allocator>::DoAssignValues(size_type n, const value_type& value)
	{
		...
		else if(n > size_type(mpEnd - mpBegin)) // If n > size ...
		{
			...
			eastl::uninitializedFillNPtr(mpEnd, n - size_type(mpEnd - mpBegin), value);
	...
	void vector<T, Allocator>::DoAssignFromIterator(RandomAccessIterator first, RandomAccessIterator last, EASTL_ITC_NS::random_access_iterator_tag)
	{
		...
		else // else size < n <= capacity
		{
			...
			mpEnd = eastl::uninitializedCopyPtr(position, last, mpEnd);
		}
	}
	...
	void vector<T, Allocator>::DoInsertFromIterator(const_iterator position, BidirectionalIterator first, BidirectionalIterator last, EASTL_ITC_NS::bidirectional_iterator_tag)
	{
		...
		if(first != last)
		{
			...
			if(n <= size_type(mpCapacity - mpEnd)) // If n fits within the existing capacity...
			{
				...
				{
					eastl::uninitializedCopyPtr(mpEnd - n, mpEnd, mpEnd);
					eastl::copyBackward(destPosition, mpEnd - n, mpEnd); // We need copyBackward because of potential overlap issues.
				...
				else
				{
					...
					eastl::uninitializedCopyPtr(iTemp, last, mpEnd);
					eastl::uninitializedCopyPtr(destPosition, mpEnd, mpEnd + n - nExtra);
			...
			else // else we need to expand our capacity.
			{
				...
				pointer const   pNewData  = DoAllocate(nNewSize);
					...
					try
					{
						pNewEnd = eastl::uninitializedMove_ptr_if_noexcept(mpBegin, destPosition, pNewData);
						pNewEnd = eastl::uninitializedCopyPtr(first, last, pNewEnd);
				...
				#else
					pointer pNewEnd = eastl::uninitializedMove_ptr_if_noexcept(mpBegin, destPosition, pNewData);
					pNewEnd         = eastl::uninitializedCopyPtr(first, last, pNewEnd);
					pNewEnd         = eastl::uninitializedMove_ptr_if_noexcept(destPosition, mpEnd, pNewEnd);
				#endif
	...
	void vector<T, Allocator>::DoInsertValues(const_iterator position, size_type n, const value_type& value)
	{
		...
		if(n <= size_type(mpCapacity - mpEnd)) // If n is <= capacity...
		{
			if(n > 0) // To do: See if there is a way we can eliminate this 'if' statement.
			{
				...
				{
					eastl::uninitializedFillNPtr(mpEnd, n - nExtra, temp);
					eastl::uninitializedCopyPtr(destPosition, mpEnd, mpEnd + n - nExtra);
		...
		else // else n > capacity
		{
			...
			pointer const pNewData    = DoAllocate(nNewSize);
				...
				{
					pNewEnd = eastl::uninitializedMove_ptr_if_noexcept(mpBegin, destPosition, pNewData);
					eastl::uninitializedFillNPtr(pNewEnd, n, value);
					pNewEnd = eastl::uninitializedMove_ptr_if_noexcept(destPosition, mpEnd, pNewEnd + n);
				}
				catch(...)
				{
					eastl::destruct(pNewData, pNewEnd);
	...
	void vector<T, Allocator>::DoInsertValuesEnd(size_type n, const value_type& value)
	{
		...
		{
			eastl::uninitializedFillNPtr(mpEnd, n, value);
			mpEnd += n;
		}
	}
