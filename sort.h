...
namespace eastl
{
	...
	namespace Internal
	{
			...
			&& !eastl::is_copy_constructible<typename iterator_traits<RandomAccessIterator>::value_type>::value>::type* = 0)
		{
			...
			quickSort_impl_helper<RandomAccessIterator, Size, value_type>(first, last, kRecursionCount);
		}
			...
			typename eastl::enable_if<eastl::is_copy_constructible<typename iterator_traits<RandomAccessIterator>::value_type>::value>::type* = 0)
		{
			...
			// copy constructors require const value_type
			quickSort_impl_helper<RandomAccessIterator, Size, Compare, const value_type>(first, last, kRecursionCount, compare);
		}
			...
			&& !eastl::is_copy_constructible<typename iterator_traits<RandomAccessIterator>::value_type>::value>::type* = 0)
		{
			...
			// move constructors require non-const value_type
			quickSort_impl_helper<RandomAccessIterator, Size, Compare, value_type>(first, last, kRecursionCount, compare);
		}
	}
	...
	void quickSort(RandomAccessIterator first, RandomAccessIterator last)
	{
		...
		if(first != last)
		{
			eastl::Internal::quickSort_impl<RandomAccessIterator, difference_type>(first, last, 2 * Internal::Log2(last - first));
	...
	void quickSort(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
	{
		...
		if(first != last)
		{
			eastl::Internal::quickSort_impl<RandomAccessIterator, difference_type, Compare>(first, last, 2 * Internal::Log2(last - first), compare);
	...
	namespace Internal
	{
							  ...
							  intptr_t& len, intptr_t& run, intptr_t& curr, intptr_t& stack_curr, StrictWeakOrdering compare)
		{
			...
			{
				insertionSort_already_started<RandomAccessIterator, StrictWeakOrdering>(first + curr, first + curr + run, first + curr + len, compare);
				len = run;
			}
	...
	// data sets are non-trivial (size >= 15). It's also the fastest sort (e.g. faster than quickSort) for 
	// the case that at at least half your data is already sorted. Otherwise, eastl::quickSort is about 10% 
	// faster than tim_sort_buffer but is not a stable sort. There are some reports that tim_sort outperforms
	// quickSort but most of these aren't taking into account that optimal quickSort implementations use
	// a hybrid approach called "introsort" (http://en.wikipedia.org/wiki/Introsort) which improves quickSort
	...
	//     - Simpler to understand than quickSort.
	//
	// Weaknesses:
	//     - User must provide a scratch buffer, otherwise the buffer is dynamically allocated during runtime.
	//     - Not as fast as quickSort for the general case of randomized data.
	...
	/// an integral type. This limits what can be sorted, but radixSort is 
	...
	namespace Internal
	{
		...
		// The radixSort implementation uses two optimizations that are not part of a typical radix sort implementation.
		...
		void radixSort_impl(RandomAccessIterator first,
			...
			IntegerType)
		{
			...
			uint32_t EASTL_PREFIX_ALIGN(EASTL_PLATFORM_PREFERRED_ALIGNMENT) bucketPosition[numBuckets];
	...
	void radixSort(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator buffer)
	{
		...
		eastl::Internal::radixSort_impl<RandomAccessIterator, ExtractKey, DigitBits>(first, last, buffer, ExtractKey(), typename ExtractKey::radix_type());
	}
	...
	inline void bubbleSort(ForwardIterator first, ForwardIterator last, StrictWeakOrdering compare)
	{
		...
		eastl::Internal::bubbleSort_impl<ForwardIterator, StrictWeakOrdering>(first, last, compare, IC());
	}
	...
	inline void bubbleSort(ForwardIterator first, ForwardIterator last)
	{
		...
		eastl::Internal::bubbleSort_impl<ForwardIterator, Less>(first, last, Less(), IC());
	}
	...
	/// sort
	/// 
	/// We use quickSort by default. See quickSort for details.
	///
	/// EASTL_DEFAULT_SORT_FUNCTION
	/// If a default sort function is specified then call it, otherwise use EASTL's default quickSort.
	/// EASTL_DEFAULT_SORT_FUNCTION must be namespace-qualified and include any necessary template
	/// parameters (e.g. eastl::combSort instead of just combSort), and it must be visible to this code. 
	...
	/// stableSort
	/// 
	/// We use mergeSort by default. See mergeSort for details.
	...
	/// If a default sort function is specified then call it, otherwise use EASTL's default mergeSort.
	...
	void stableSort(RandomAccessIterator first, RandomAccessIterator last, StrictWeakOrdering compare)
	{
			...
			EASTL_DEFAULT_STABLE_SORT_FUNCTION(first, last, *getDefaultAllocator(0), compare);
		#else
	...
	{
		#if defined(EASTL_DEFAULT_STABLE_SORT_FUNCTION)
			EASTL_DEFAULT_STABLE_SORT_FUNCTION(first, last, *getDefaultAllocator(0));
		#else
