...
namespace eastl
{
	...
	template <typename T> 
	inline void swap(T& a, T& b) EASTL_NOEXCEPT_IF(eastl::is_nothrow_move_constructible<T>::value && eastl::is_nothrow_move_assignable<T>::value)
	{
	...
	inline void iterSwap(ForwardIterator1 a, ForwardIterator2 b)
	{
		...
		eastl::iterSwap_impl<eastl::type_and<eastl::is_same<value_type_a, value_type_b>::value, eastl::is_same<value_type_a&, reference_a>::value, eastl::is_same<value_type_b&, reference_b>::value >::value >::iterSwap(a, b);
	}
	...
	swapRanges(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2)
	{
		for(; first1 != last1; ++first1, ++first2)
			iterSwap(first1, first2); // Don't prefix swap with eastl:: as we want to allow user-defined swaps via argument-dependent lookup.
		return first2;
	}
	...
	inline void
	swap(T (&a)[N], T (&b)[N]) EASTL_NOEXCEPT_IF(eastl::is_nothrow_swappable<T>::value)
	{
		eastl::swapRanges(a, a + N, b);
	}
	...
	///
	template <class T>
	EA_CONSTEXPR typename eastl::add_const<T>::type& as_const(T& t) EASTL_NOEXCEPT
		{ return t; }
	...
	struct pair
	{
		...
		EA_CPP14_CONSTEXPR pair(const pair<U, V>& p, typename eastl::enable_if<eastl::is_convertible<const U&, T1>::value && eastl::is_convertible<const V&, T2>::value>::type* = 0)
			...
			EA_CPP14_CONSTEXPR pair(const T1& x, V&& y)
				...
				EA_CPP14_CONSTEXPR pair(pair&& p) EASTL_NOEXCEPT_IF(eastl::is_nothrow_move_constructible<T1>::value && eastl::is_nothrow_move_constructible<T2>::value)
		...
		pair& operator=(const pair& p) EASTL_NOEXCEPT_IF(eastl::is_nothrow_copy_assignable<T1>::value && eastl::is_nothrow_copy_assignable<T2>::value)
		{
			first  = p.first;
			second = p.second;
			return *this;
		...
		#if EASTL_MOVE_SEMANTICS_ENABLED
			pair& operator=(pair&& p) EASTL_NOEXCEPT_IF(eastl::is_nothrow_move_assignable<T1>::value && eastl::is_nothrow_move_assignable<T2>::value)
			{
				first  = eastl::forward<T1>(p.first);
				second = eastl::forward<T2>(p.second);
				return *this;
		...
		void swap(pair& p) EASTL_NOEXCEPT_IF(eastl::is_nothrow_swappable<T1>::value && eastl::is_nothrow_swappable<T2>::value)
		{
			eastl::iterSwap(&first,  &p.first);
			eastl::iterSwap(&second, &p.second);
		}
	};
	...
	/// the useFirst template implementation.
	///
	/// The existance of useSelf may seem odd, given that it does nothing,
	/// but these kinds of things are useful, virtually required, for optimal 
	/// generic programming.
	///
	template <typename T>
	struct useSelf             // : public unary_function<T, T> // Perhaps we want to make it a subclass of unary_function.
	{
	...
	/// This is a template policy implementation; it is an alternative to 
	/// the useSelf template implementation. This is the same thing as the
	...
	///////////////////////////////////////////////////////////////////////
	/// makePair / makePairRef
	///
	/// makePair is the same as std::makePair specified by the C++ standard.
	...
	/// we provide the makePairRef function below, though C++11 move support
	/// makes that no longer necessary.
	/// 
	/// Note: You don't usually need to use makePair in order to make a pair. 
	/// The following code is equivalent, and the latter avoids one more level of inlining:
	///     return makePair(charPtr, charPtr);
	...
	///
		...
		// Without the following, VC++ fails to compile code like this: pair<const char*, int> p = eastl::makePair<const char*, int>("hello", 0);
		// We define a const reference version alternative to the above. "hello" is of type char const(&)[6] (array of 6 const chars), 
		// but VC++ decays it to const char* and allows this makePair to be called with that. VC++ fails below with makePair("hello", "people") 
		// because you can't assign arrays and until we have a better solution we just disable this makePair specialization for when T1 or T2 
		// are of type char const(&)[].
		#if defined(_MSC_VER)
			template <typename T1, typename T2>
			EA_CPP14_CONSTEXPR inline pair<T1, T2> makePair(const T1& a, const T2& b, typename eastl::enable_if<!eastl::is_array<T1>::value && !eastl::is_array<T2>::value>::type* = 0)
			{
