...
namespace eastl
{
	...
	/// 
	/// Note: You don't need to use makePair in order to make a pair, it's a convenience for deducing the element types. 
	...
	///     auto p2 = makePair(2, "deduce types");
	...
	// Note: DO NOT pass type parameters to makePair().
	// eg.
	//   const int myInt = 0;
	//   const char* myCStr = "example";
	//   auto pair1 = eastl::makePair<int, eastl::string>(myInt, myCStr);
	//                                ^^^^^^^^^^^^^^^^^^^^--- wrong.
	// 
	// use one of the following, depending on the intended pair type: 
	//   eastl::pair<int, const char*> pair2 = eastl::makePair(myInt, myCStr);
	//   eastl::pair<int, eastl::string> pair3(myInt, myCStr);
	//
#if defined(_MSC_VER)
	template <typename T1, typename T2, typename DeduceT1, typename DeduceT2>
	EASTL_REMOVE_AT_2024_SEPT EA_CPP14_CONSTEXPR inline pair<T1, T2> makePair(
		const DeduceT1& a,
		const DeduceT2& b,
	...
	// use makePair() instead. they are equivalent.
	template <typename T1, typename T2>
	...
	makePairRef(T1&& a, T2&& b)
	{
		...
		template <typename T1, typename T2>
		EA_CONSTEXPR T1& get(pair<T1, T2>& p) EASTL_NOEXCEPT
		{
			return internal::GetPair<0>::get(p);
		}

		template <typename T2, typename T1>
		EA_CONSTEXPR T2& get(pair<T1, T2>& p) EASTL_NOEXCEPT
		{
			return internal::GetPair<1>::get(p);
		}

		template <typename T1, typename T2>
		EA_CONSTEXPR const T1& get(const pair<T1, T2>& p) EASTL_NOEXCEPT
		{
			return internal::GetPair<0>::get(p);
		}

		template <typename T2, typename T1>
		EA_CONSTEXPR const T2& get(const pair<T1, T2>& p) EASTL_NOEXCEPT
		{
			return internal::GetPair<1>::get(p);
		}
